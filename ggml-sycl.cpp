...
/*
Following definition copied from DPCT head files, which are used by
ggml-sycl.cpp
*/
...
class device_info {
 public:
  ...
  [[deprecated]] void set_max_work_item_sizes(
...
#define GGML_SYCL_DEBUG(...)                    \
  do {                                          \
    if (g_ggml_sycl_debug) printf(__VA_ARGS__); \
  } while (0)
...
static void ggml_sycl_error(const char *stmt, const char *func,
...
#define SYCL_CHECK(err)                                   \
  do {                                                    \
    auto err_ = (err);                                    \
    if (err_ != 0)                                        \
      ggml_sycl_error(#err, __func__, __FILE__, __LINE__, \
...
void log_tensor_with_cnt(const char *name, struct ggml_tensor *src,
                         int stop_cnt);

static __dpct_inline__ int get_int_from_int8(const int8_t *x8, const int &i32) {
  const uint16_t *x16 =
      (const uint16_t *)(x8 + sizeof(int) *
                                  i32);  // assume at least 2 byte alignment
...
static __dpct_inline__ int get_int_from_int8_aligned(const int8_t *x8,
                                                     const int &i32) {
  return *((const int *)(x8 + sizeof(int) *
                                  i32));  // assume at least 4 byte alignment
}
...
typedef void (*dequantize_kernel_t)(const void *vx, const int ib, const int iqs,
                                    dfloat2 &v);
typedef void (*dot_kernel_k_t)(const void *__restrict__ vx, const int ib,
                               const int iqs, const float *__restrict__ y,
                               float &v);
typedef void (*cpy_kernel_t)(const char *cx, char *cdst);
typedef void (*ggml_sycl_func_t)(const ggml_tensor *src0,
...
static_assert(sizeof(block_q8_1) == 2 * sizeof(ggml_fp16_t) + QK8_0,
...
// static_assert(sizeof(block_q3_K) == sizeof(ggml_fp16_t) + QK_K / 4 + QK_K / 8
// + K_SCALE_SIZE, "wrong q3_K block size/padding");
...
static_assert(sizeof(block_q4_K) == sizeof(half2) + QK_K / 2 + 2,
...
static_assert(sizeof(block_q4_K) ==
                  2 * sizeof(ggml_fp16_t) + 3 * QK_K / 64 + QK_K / 2,
              "wrong q4_K block size/padding");
#endif
...
#else
static_assert(K_QUANTS_PER_ITERATION == 1 || K_QUANTS_PER_ITERATION == 2,
...
struct ggml_tensor_extra_gpu {
  void *data_device[GGML_SYCL_MAX_DEVICES];  // 1 pointer for each device for
                                             // split tensors
  dpct::event_ptr
...
static sycl_device_capabilities g_device_caps[GGML_SYCL_MAX_DEVICES] = {
    {0, false, 0, -1}};
...
static sycl_device_id2index g_sycl_device_id2index[GGML_SYCL_MAX_DEVICES] = {
    {-1}};
...
int get_main_device() { return g_main_device; }
...
void log_ggml_var_device(const char *name, float *src, size_t total_elements,
                         bool src_on_device) {
...
void print_ggml_tensor(const char *name, struct ggml_tensor *src) {
  ...
  const bool src_on_device = src->backend == GGML_BACKEND_GPU ||
                             src->backend == GGML_BACKEND_GPU_SPLIT;
  float *src_data = NULL;
  if (src_on_device) {
...
static __dpct_inline__ sycl::float2 warp_reduce_sum(
    sycl::float2 a, const sycl::nd_item<3> &item_ct1) {
#pragma unroll
  for (int mask = 16; mask > 0; mask >>= 1) {
    a.x() +=
        dpct::permute_sub_group_by_xor(item_ct1.get_sub_group(), a.x(), mask);
    a.y() +=
        dpct::permute_sub_group_by_xor(item_ct1.get_sub_group(), a.y(), mask);
  }
  return a;
}
...
template <float (*bin_op)(const float, const float), typename src0_t,
          typename src1_t, typename dst_t>
static void k_bin_bcast(const src0_t *src0, const src1_t *src1, dst_t *dst,
                        int ne0, int ne1, int ne2, int ne3, int ne10, int ne11,
                        int ne12, int ne13,
  ...
  for (int i0 = i0s; i0 < ne0;
    ...
    dst_row[i0] =
        (dst_t)bin_op(src0 ? (float)src0_row[i0] : 0.0f, (float)src1_row[i10]);
  }
}

template <float (*bin_op)(const float, const float), typename src0_t,
          typename src1_t, typename dst_t>
static void k_bin_bcast_unravel(const src0_t *src0, const src1_t *src1,
                                dst_t *dst, int ne0, int ne1, int ne2, int ne3,
...
static void gelu_f32(const float *x, float *dst, const int k,
  ...
  dst[i] =
      0.5f * xi *
      (1.0f + sycl::tanh(SQRT_2_OVER_PI * xi * (1.0f + GELU_COEF_A * xi * xi)));
}
...
static void leaky_relu_f32(const float *x, float *dst, const int k,
...
static void norm_f32(const float *x, float *dst, const int ncols,
                     const float eps, const sycl::nd_item<3> &item_ct1,
...
static void concat_f32(const float *x, const float *y, float *dst,
...
static void upscale_f32(const float *x, float *dst, const int ne00,
                        const int nb02, const int scale_factor,
                        const sycl::nd_item<3> &item_ct1) {
...
static void pad_f32(const float *x, float *dst, const int ne0, const int ne00,
...
static void dequantize_block_q2_K(const void *__restrict__ vx,
  ...
  y[l + 32] = dall * (x[i].scales[is + 2] & 0xF) * ((q >> 2) & 3) -
              dmin * (x[i].scales[is + 2] >> 4);
  y[l + 64] = dall * (x[i].scales[is + 4] & 0xF) * ((q >> 4) & 3) -
              dmin * (x[i].scales[is + 4] >> 4);
  y[l + 96] = dall * (x[i].scales[is + 6] & 0xF) * ((q >> 6) & 3) -
              dmin * (x[i].scales[is + 6] >> 4);
#else
  ...
  y[0] = dall * (x[i].scales[is + 0] & 0xF) * ((q >> 0) & 3) -
         dmin * (x[i].scales[is + 0] >> 4);
  y[32] = dall * (x[i].scales[is + 2] & 0xF) * ((q >> 4) & 3) -
          dmin * (x[i].scales[is + 2] >> 4);
#endif
}

template <typename dst_t>
static void dequantize_block_q3_K(const void *__restrict__ vx,
                                  dst_t *__restrict__ yy,
                                  const sycl::nd_item<3> &item_ct1) {
  const int i = item_ct1.get_group(2);
  ...
  int8_t us = is < 4 ? (x[i].scales[is - 0] & 0xF) |
                           (((x[i].scales[is + 8] >> 0) & 3) << 4)
              : is < 8 ? (x[i].scales[is - 0] & 0xF) |
                             (((x[i].scales[is + 4] >> 2) & 3) << 4)
              : is < 12 ? (x[i].scales[is - 8] >> 4) |
  ...
  if (is == 0) {
    y[0] = d * ((x[i].scales[0] & 0xF) - 8) *
           ((int8_t)((q >> 0) & 3) - ((h >> 0) & 1 ? 0 : 4));
    y[32] = d * ((x[i].scales[1] & 0xF) - 8) *
            ((int8_t)((q >> 4) & 3) - ((h >> 4) & 1 ? 0 : 4));
  } else {
    y[0] = d * ((x[i].scales[0] >> 4) - 8) *
           ((int8_t)((q >> 0) & 3) - ((h >> 0) & 1 ? 0 : 4));
    y[32] = d * ((x[i].scales[1] >> 4) - 8) *
            ((int8_t)((q >> 4) & 3) - ((h >> 4) & 1 ? 0 : 4));
  }
#endif
}
...
template <typename dst_t>
static void dequantize_block_q4_K(const void *__restrict__ vx,
                                  dst_t *__restrict__ yy,
                                  const sycl::nd_item<3> &item_ct1) {
  ...
  y[tid + 0] =
      d * (x[i].scales[0] & 0xF) * (q[tid] & 0xF) - m * (x[i].scales[0] >> 4);
  y[tid + 32] =
      d * (x[i].scales[1] & 0xF) * (q[tid] >> 4) - m * (x[i].scales[1] >> 4);
#endif
}

template <typename dst_t>
static void dequantize_block_q5_K(const void *__restrict__ vx,
                                  dst_t *__restrict__ yy,
                                  const sycl::nd_item<3> &item_ct1) {
...
template <typename dst_t>
static void dequantize_block_q6_K(const void *__restrict__ vx,
                                  dst_t *__restrict__ yy,
                                  const sycl::nd_item<3> &item_ct1) {
...
static void dequantize_block_iq2_xxs(const void *__restrict__ vx,
  ...
  sycl::ext::oneapi::experimental::printf("q2: %d, %d %d %d %d\n", tid, q2[0], q2[1], q2[2], q2[3]);
...
static void dequantize_mul_mat_vec_q2_k(const void *__restrict__ vx,
  ...
  for (int i = ix; i < num_blocks_per_row; i += K_QUANTS_PER_ITERATION) {
    ...
    for (int l = 0; l < K_QUANTS_PER_ITERATION; ++l) {
      ...
      sum2 += y[l + 0] * m[0] + y[l + 32] * m[2] + y[l + 64] * m[4] +
              y[l + 96] * m[6] + y[l + 16] * m[1] + y[l + 48] * m[3] +
              y[l + 80] * m[5] + y[l + 112] * m[7];
    }
    tmp += dall * sum1 - dmin * sum2;
  }
#else
  const int tid =
      threadIdx.x / (2 * K_QUANTS_PER_ITERATION);             // 0...15 or 0...7
  ...
  for (int i = ix; i < num_blocks_per_row; i += 2 * K_QUANTS_PER_ITERATION) {
    ...
    for (int l = 0; l < K_QUANTS_PER_ITERATION; ++l) {
      ...
      sum1 += y[l + 0] * d[0] * ((ql >> 0) & 3) +
              ...
              y[l + 32] * d[2] * ((ql >> 4) & 3) +
              y[l + 48] * d[3] * ((ql >> 6) & 3);
      sum2 += y[l + 0] * d[4] + y[l + 16] * d[5] + y[l + 32] * d[6] +
              y[l + 48] * d[7];
    }
...
static void dequantize_mul_mat_vec_q3_k(const void *__restrict__ vx,
  ...
  for (int i = ix; i < num_blocks_per_row; i += K_QUANTS_PER_ITERATION) {
    ...
    utmp[0] = ((a[0] >> s_shift) & kmask2) |
              (((a[4] >> (s_shift + 0)) & kmask1) << 4);
    utmp[1] = ((a[1] >> s_shift) & kmask2) |
              (((a[5] >> (s_shift + 0)) & kmask1) << 4);
    utmp[2] = ((a[2] >> s_shift) & kmask2) |
              (((a[4] >> (s_shift + 2)) & kmask1) << 4);
    ...
    for (int l = 0; l < n; ++l) {
      sum += y[l + 0] * (s[0] - 32) *
             ...
             y[l + 96] * (s[6] - 32) *
                 (((q[l] >> 6) & 3) - (h[l] & (m << 3) ? 0 : 4));
      sum += y[l + 16] * (s[1] - 32) *
                 (((q[l + 16] >> 0) & 3) - (h[l + 16] & (m << 0) ? 0 : 4)) +
             y[l + 48] * (s[3] - 32) *
                 (((q[l + 16] >> 2) & 3) - (h[l + 16] & (m << 1) ? 0 : 4)) +
             y[l + 80] * (s[5] - 32) *
                 (((q[l + 16] >> 4) & 3) - (h[l + 16] & (m << 2) ? 0 : 4)) +
             y[l + 112] * (s[7] - 32) *
                 (((q[l + 16] >> 6) & 3) - (h[l + 16] & (m << 3) ? 0 : 4));
    }
    tmp += d * sum;
  }
#else
  ...
  for (int i = ix; i < num_blocks_per_row; i += 2 * K_QUANTS_PER_ITERATION) {
    ...
    for (int l = 0; l < K_QUANTS_PER_ITERATION; ++l) {
      ...
      const uint8_t ql = q[l];
      sum += y[l + 0] * dall * ((s[0] & 0xF) - 8) *
                 ((int8_t)((ql >> 0) & 3) - ((hl >> 0) & 1 ? 0 : 4)) +
             y[l + 16] * dall * ((s[0] >> 4) - 8) *
                 ((int8_t)((ql >> 2) & 3) - ((hl >> 2) & 1 ? 0 : 4)) +
             y[l + 32] * dall * ((s[1] & 0xF) - 8) *
                 ((int8_t)((ql >> 4) & 3) - ((hl >> 4) & 1 ? 0 : 4)) +
             y[l + 48] * dall * ((s[1] >> 4) - 8) *
                 ((int8_t)((ql >> 6) & 3) - ((hl >> 6) & 1 ? 0 : 4));
    }
    tmp += sum;
  }
#endif
...
static void dequantize_mul_mat_vec_q4_k(const void *__restrict__ vx,
  ...
  const int im =
      il / 2;  // 0 or 1. 0 computes 0,32 + 128,160, 1 computes 64,96 + 192,224
  const int in = il % 2;
  ...
  for (int i = ix; i < num_blocks_per_row; i += K_QUANTS_PER_ITERATION) {
    ...
    for (int l = 0; l < 4; ++l) {
      ...
      s.z() += y2[l] * q4[l + 8];
      s.w() += y2[l + 32] * q4[l + 12];
      smin += y1[l] * sc[2] + y1[l + 32] * sc[3] + y2[l] * sc[6] +
              y2[l + 32] * sc[7];
    }
    tmp += dall * (s.x() * sc[0] + s.y() * sc[1] * 1.f / 16.f + s.z() * sc[4] +
                   s.w() * sc[5] * 1.f / 16.f) -
           dmin * smin;
#else
...
static void dequantize_mul_mat_vec_q5_k(const void *__restrict__ vx,
  ...
  for (int i = ix; i < num_blocks_per_row; i += 2 * K_QUANTS_PER_ITERATION) {
    ...
    for (int j = 0; j < K_QUANTS_PER_ITERATION; ++j) {
      ...
      sum +=
          y[j + 0] * d * s[0] * ((q[j + 0] & 0xF) - ((h >> 0) & 1 ? 0 : 16)) +
...
static void dequantize_mul_mat_vec_q6_k(const void *__restrict__ vx,
...
#if K_QUANTS_PER_ITERATION == 1
    float sum = y[0] * s[0] * d *
                    ((int8_t)((ql[0] & 0xF) | ((qh[0] & 0x03) << 4)) - 32) +
                y[16] * s[1] * d *
                    ((int8_t)((ql[16] & 0xF) | ((qh[16] & 0x03) << 4)) - 32) +
                y[32] * s[2] * d *
                    ((int8_t)((ql[32] & 0xF) | ((qh[0] & 0x0c) << 2)) - 32) +
                y[48] * s[3] * d *
                    ((int8_t)((ql[48] & 0xF) | ((qh[16] & 0x0c) << 2)) - 32) +
                y[64] * s[4] * d *
                    ((int8_t)((ql[0] >> 4) | ((qh[0] & 0x30) >> 0)) - 32) +
                y[80] * s[5] * d *
                    ((int8_t)((ql[16] >> 4) | ((qh[16] & 0x30) >> 0)) - 32) +
                y[96] * s[6] * d *
                    ((int8_t)((ql[32] >> 4) | ((qh[0] & 0xc0) >> 2)) - 32) +
                y[112] * s[7] * d *
                    ((int8_t)((ql[48] >> 4) | ((qh[16] & 0xc0) >> 2)) - 32);
    tmp += sum;
#else
    float sum = 0;
    for (int l = 0; l < 4; ++l) {
  ...
  for (int i = ix; i < num_blocks_per_row; i += 2 * K_QUANTS_PER_ITERATION) {
    ...
    for (int j = 0; j < K_QUANTS_PER_ITERATION; ++j) {
      sum += y[j + 0] * s[0] * d *
                 ((int8_t)((ql[j + 0] & 0xF) | ((qh[j] & 0x03) << 4)) - 32) +
             y[j + 16] * s[1] * d *
                 ((int8_t)((ql[j + 16] & 0xF) | ((qh[j] & 0x0c) << 2)) - 32) +
             y[j + 32] * s[2] * d *
                 ((int8_t)((ql[j + 0] >> 4) | ((qh[j] & 0x30) >> 0)) - 32) +
             y[j + 48] * s[3] * d *
                 ((int8_t)((ql[j + 16] >> 4) | ((qh[j] & 0xc0) >> 2)) - 32);
    }
    tmp += sum;
  }
...
static void convert_f16(const void *vx, const int ib, const int iqs,
                        dfloat2 &v) {
...
static void convert_f32(const void *vx, const int ib, const int iqs,
                        dfloat2 &v) {
...
static void quantize_q8_1(const float *__restrict__ x, void *__restrict__ vy,
...
static void k_get_rows(
    ...
    /*size_t nb00,*/ size_t nb01, size_t nb02, size_t nb03, size_t s10,
    size_t s11, size_t s12, const sycl::nd_item<3> &item_ct1 /*, size_t s13*/) {
  ...
  const void *src0_row =
      (const char *)src0 + i01 * nb01 + i11 * nb02 + i12 * nb03;
...
static void k_get_rows_float(
  ...
  dst_t *dst_row = dst + i10 * s1 + i11 * s2 + i12 * s3;
  const src0_t *src0_row = (const src0_t *)((const char *)src0 + i01 * nb01 +
                                            i11 * nb02 + i12 * nb03);
...
// VDR = vec dot ratio, how many contiguous integers each thread processes when
// the vec dot kernel is called MMVQ = mul_mat_vec_q, MMQ = mul_mat_q
...
#pragma unroll
  ...
  // scale second part of sum by QI8_1/(vdr * QR4_1) to compensate for multiple
  // threads adding it
...
template <int vdr>
static __dpct_inline__ float vec_dot_q5_0_q8_1_impl(const int *vl,
                                                    const int *vh, const int *u,
                                                    const float &d5,
                                                    const sycl::half2 &ds8) {
  int sumi = 0;
...
#pragma unroll
  for (int i = 0; i < vdr; ++i) {
    int vi0 = (vl[i] >> 0) &
              0x0F0F0F0F;  // lower 4 qs bits, still need qh as 5th bits
    ...
    int vi1 = (vl[i] >> 4) &
              0x0F0F0F0F;  // upper 4 qs bits, still need qh as 5th bits
...
template <int vdr>
static __dpct_inline__ float vec_dot_q5_1_q8_1_impl(const int *vl,
                                                    const int *vh, const int *u,
                                                    const sycl::half2 &dm5,
                                                    const sycl::half2 &ds8) {
  int sumi = 0;
...
#pragma unroll
  ...
  // scale second part of sum by QI5_1 / vdr to compensate for multiple threads
  // adding it
...
#pragma unroll
  ...
  // scale second part of sum by QI8_1/ vdr to compensate for multiple threads
  // adding it
...
static __dpct_inline__ float vec_dot_q2_K_q8_1_impl_mmq(
    const int *__restrict__ v, const int *__restrict__ u,
    const uint8_t *__restrict__ scales, const sycl::half2 &dm2,
    const float &d8) {
  int sumi_d = 0;
  int sumi_m = 0;
...
// contiguous u/y values
static __dpct_inline__ float vec_dot_q3_K_q8_1_impl_mmq(
    const int *__restrict__ v, const int *__restrict__ u,
    const int8_t *__restrict__ scales, const float &d3, const float &d8) {
  int sumi = 0;
...
// contiguous u/y values
static __dpct_inline__ float vec_dot_q6_K_q8_1_impl_mmq(
    const int *__restrict__ v, const int *__restrict__ u,
...
static __dpct_inline__ float vec_dot_q4_0_q8_1(
...
static __dpct_inline__ void load_tiles_q4_0(
...
static __dpct_inline__ float vec_dot_q4_1_q8_1(
    const void *__restrict__ vbq, const block_q8_1 *__restrict__ bq8_1,
    const int &iqs) {
...
template <int mmq_y>
static __dpct_inline__ void allocate_tiles_q4_1(int **x_ql, sycl::half2 **x_dm,
                                                int **x_qh, int **x_sc,
                                                int *tile_x_qs_q4_1,
...
template <int mmq_y, int nwarps, bool need_check>
static __dpct_inline__ void load_tiles_q4_1(
    const void *__restrict__ vx, int *__restrict__ x_ql,
    sycl::half2 *__restrict__ x_dm, int *__restrict__ x_qh,
    int *__restrict__ x_sc, const int &i_offset, const int &i_max, const int &k,
    const int &blocks_per_row) {
...
static __dpct_inline__ float vec_dot_q5_0_q8_1(
    const void *__restrict__ vbq, const block_q8_1 *__restrict__ bq8_1,
    const int &iqs) {
...
template <int mmq_y>
static __dpct_inline__ void allocate_tiles_q5_0(int **x_ql, sycl::half2 **x_dm,
                                                int **x_qh, int **x_sc,
                                                int *tile_x_ql_q5_0,
                                                float *tile_x_d_q5_0) {
  (void)x_qh;
...
template <int mmq_y, int nwarps, bool need_check>
static __dpct_inline__ void load_tiles_q5_0(
    const void *__restrict__ vx, int *__restrict__ x_ql,
    sycl::half2 *__restrict__ x_dm, int *__restrict__ x_qh,
    int *__restrict__ x_sc, const int &i_offset, const int &i_max, const int &k,
    const int &blocks_per_row) {
...
#pragma unroll
  for (int i0 = 0; i0 < mmq_y; i0 += nwarps) {
    ...
    qs0 = dpct::vectorized_binary<sycl::char4>(qs0, 0x10101010,
                                               dpct::sub_sat());  // subtract 16
    ...
    qs1 = dpct::vectorized_binary<sycl::char4>(qs1, 0x10101010,
                                               dpct::sub_sat());  // subtract 16
...
static __dpct_inline__ float vec_dot_q5_1_q8_1(
    const void *__restrict__ vbq, const block_q8_1 *__restrict__ bq8_1,
    const int &iqs) {
...
template <int mmq_y>
static __dpct_inline__ void allocate_tiles_q5_1(int **x_ql, sycl::half2 **x_dm,
                                                int **x_qh, int **x_sc,
                                                int *tile_x_ql_q5_1,
...
template <int mmq_y, int nwarps, bool need_check>
static __dpct_inline__ void load_tiles_q5_1(
    const void *__restrict__ vx, int *__restrict__ x_ql,
    sycl::half2 *__restrict__ x_dm, int *__restrict__ x_qh,
    int *__restrict__ x_sc, const int &i_offset, const int &i_max, const int &k,
    const int &blocks_per_row) {
...
static __dpct_inline__ float vec_dot_q8_0_q8_1(
    const void *__restrict__ vbq, const block_q8_1 *__restrict__ bq8_1,
    const int &iqs) {
...
template <int mmq_y>
static __dpct_inline__ void allocate_tiles_q8_0(int **x_ql, sycl::half2 **x_dm,
                                                int **x_qh, int **x_sc,
                                                int *tile_x_qs_q8_0,
                                                float *tile_x_d_q8_0) {
  (void)x_qh;
...
template <int mmq_y, int nwarps, bool need_check>
static __dpct_inline__ void load_tiles_q8_0(
    const void *__restrict__ vx, int *__restrict__ x_ql,
    sycl::half2 *__restrict__ x_dm, int *__restrict__ x_qh,
    int *__restrict__ x_sc, const int &i_offset, const int &i_max, const int &k,
    const int &blocks_per_row) {
...
static __dpct_inline__ float vec_dot_q2_K_q8_1(
    const void *__restrict__ vbq, const block_q8_1 *__restrict__ bq8_1,
    const int &iqs) {
...
template <int mmq_y>
static __dpct_inline__ void allocate_tiles_q2_K(int **x_ql, sycl::half2 **x_dm,
                                                int **x_qh, int **x_sc,
                                                int *tile_x_ql_q2_K,
...
template <int mmq_y, int nwarps, bool need_check>
static __dpct_inline__ void load_tiles_q2_K(
    const void *__restrict__ vx, int *__restrict__ x_ql,
    sycl::half2 *__restrict__ x_dm, int *__restrict__ x_qh,
    int *__restrict__ x_sc, const int &i_offset, const int &i_max, const int &k,
    const int &blocks_per_row) {
...
#pragma unroll
  for (int i0 = 0; i0 < mmq_y; i0 += nwarps * 4) {
    ...
    const block_q2_K *bxi =
        bx0 + i * blocks_per_row + (k % (WARP_SIZE / 4)) / (QI2_K / 4);

    x_sc[i * (WARP_SIZE / 4) + i / 4 + k % (WARP_SIZE / 4)] =
...
static __dpct_inline__ float vec_dot_q2_K_q8_1_mul_mat(
  ...
  const int kqsx = i * (WARP_SIZE + 1) + kbx * QI2_K +
                   (QI2_K / 2) * (ky / (2 * QI2_K)) + ky % (QI2_K / 2);
...
#pragma unroll
  ...
  const uint8_t *scales =
      ((const uint8_t *)&x_sc[i * (WARP_SIZE / 4) + i / 4 + kbx * 4]) + ky / 4;
...
static __dpct_inline__ float vec_dot_q3_K_q8_1(
    const void *__restrict__ vbq, const block_q8_1 *__restrict__ bq8_1,
    const int &iqs) {
...
template <int mmq_y>
static __dpct_inline__ void allocate_tiles_q3_K(
    int **x_ql, sycl::half2 **x_dm, int **x_qh, int **x_sc, int *tile_x_ql_q3_K,
...
template <int mmq_y, int nwarps, bool need_check>
static __dpct_inline__ void load_tiles_q3_K(
    const void *__restrict__ vx, int *__restrict__ x_ql,
    sycl::half2 *__restrict__ x_dm, int *__restrict__ x_qh,
    int *__restrict__ x_sc, const int &i_offset, const int &i_max, const int &k,
    const int &blocks_per_row) {
...
#pragma unroll
  for (int i0 = 0; i0 < mmq_y; i0 += nwarps * 2) {
    ...
    const block_q3_K *bxi =
        bx0 + i * blocks_per_row + (k % (WARP_SIZE / 2)) / (QI3_K / 2);

    // invert the mask with ~ so that a 0/1 results in 4/0 being subtracted
    x_qh[i * (WARP_SIZE / 2) + i / 2 + k % (WARP_SIZE / 2)] =
...
#pragma unroll
  for (int i0 = 0; i0 < mmq_y; i0 += nwarps * 4) {
    ...
    const block_q3_K *bxi =
        bx0 + i * blocks_per_row + (k % (WARP_SIZE / 4)) / (QI3_K / 4);
    ...
    const int sc_low =
        (get_int_from_uint8(bxi->scales, ksc_low) >> shift_low) & 0x0F0F0F0F;

    const int ksc_high = QI3_K / 8;
    const int shift_high = 2 * ksc;
    const int sc_high =
        ((get_int_from_uint8(bxi->scales, ksc_high) >> shift_high) << 4) &
        0x30303030;
...
static __dpct_inline__ float vec_dot_q3_K_q8_1_mul_mat(
  ...
  const int8_t *scales =
      ((const int8_t *)(x_sc + i * (WARP_SIZE / 4) + i / 4 + kbx * 4)) + ky / 4;

  int v[QR3_K * VDR_Q3_K_Q8_1_MMQ];

#pragma unroll
  for (int l = 0; l < QR3_K * VDR_Q3_K_Q8_1_MMQ; ++l) {
    const int kqsx = i * (WARP_SIZE + 1) + kbx * QI3_K +
                     (QI3_K / 2) * (ky / (2 * QI3_K)) + ky % (QI3_K / 2);
    const int shift = 2 * ((ky % 32) / 8);
    const int vll = (x_ql[kqsx + l] >> shift) & 0x03030303;

    const int vh =
        x_qh[i * (WARP_SIZE / 2) + i / 2 + kbx * (QI3_K / 2) + (ky + l) % 8] >>
        ((ky + l) / 8);
...
#if __SYCL_ARCH__ >= \
    VER_4VEC  // lowest compute capability for integer intrinsics
  const block_q4_K *bq4_K = (const block_q4_K *)vbq;
...
template <int mmq_y, int nwarps, bool need_check>
static __dpct_inline__ void load_tiles_q4_K(
    const void *__restrict__ vx, int *__restrict__ x_ql,
    sycl::half2 *__restrict__ x_dm, int *__restrict__ x_qh,
    int *__restrict__ x_sc, const int &i_offset, const int &i_max, const int &k,
    const int &blocks_per_row) {
...
static __dpct_inline__ float vec_dot_q5_K_q8_1(
    const void *__restrict__ vbq, const block_q8_1 *__restrict__ bq8_1,
    const int &iqs) {
#ifndef GGML_QKK_64
...
#pragma unroll
  ...
  const int v1 =
      (((vh << 4) & 0x10101010) ^ 0x10101010) | ((vl1 >> 0) & 0x0f0f0f0f);
  const int v2 =
      (((vh << 2) & 0x10101010) ^ 0x10101010) | ((vl2 >> 0) & 0x0f0f0f0f);
  const int v3 =
      (((vh >> 0) & 0x10101010) ^ 0x10101010) | ((vl1 >> 4) & 0x0f0f0f0f);
  const int v4 =
      (((vh >> 2) & 0x10101010) ^ 0x10101010) | ((vl2 >> 4) & 0x0f0f0f0f);

  const float sumf_d =
      d8_1 * (__dp4a(ui1, v1, 0) * s[0] + __dp4a(ui2, v2, 0) * s[1]) +
...
template <int mmq_y>
static __dpct_inline__ void allocate_tiles_q5_K(int **x_ql, sycl::half2 **x_dm,
                                                int **x_qh, int **x_sc,
                                                int *tile_x_ql_q5_K,
...
template <int mmq_y, int nwarps, bool need_check>
static __dpct_inline__ void load_tiles_q5_K(
    const void *__restrict__ vx, int *__restrict__ x_ql,
    sycl::half2 *__restrict__ x_dm, int *__restrict__ x_qh,
    int *__restrict__ x_sc, const int &i_offset, const int &i_max, const int &k,
    const int &blocks_per_row) {
...
#pragma unroll
  for (int i0 = 0; i0 < mmq_y; i0 += nwarps * 8) {
    ...
    const block_q5_K *bxi =
        bx0 + i * blocks_per_row + (k % (WARP_SIZE / 8)) / (QI5_K / 8);
...
static __dpct_inline__ float vec_dot_q6_K_q8_1(
    const void *__restrict__ vbq, const block_q8_1 *__restrict__ bq8_1,
    const int &iqs) {
  ...
  const int vh =
      get_int_from_uint8(
          bq6_K->qh, (QI6_K / 4) * (iqs / (QI6_K / 2)) + iqs % (QI6_K / 4)) >>
      vh_shift;
...
template <int mmq_y>
static __dpct_inline__ void allocate_tiles_q6_K(int **x_ql, sycl::half2 **x_dm,
                                                int **x_qh, int **x_sc,
                                                int *tile_x_ql,
...
template <int mmq_y, int nwarps, bool need_check>
static __dpct_inline__ void load_tiles_q6_K(
    const void *__restrict__ vx, int *__restrict__ x_ql,
    sycl::half2 *__restrict__ x_dm, int *__restrict__ x_qh,
    int *__restrict__ x_sc, const int &i_offset, const int &i_max, const int &k,
    const int &blocks_per_row) {
...
#pragma unroll
  for (int i0 = 0; i0 < mmq_y; i0 += nwarps) {
    ...
    const int qh = get_int_from_uint8(
        bxi->qh, (QI6_K / 4) * (kqsx / (QI6_K / 2)) + kqsx % (QI6_K / 4));
    const int qh0 =
        ((qh >> (2 * ((kqsx % (QI6_K / 2)) / (QI6_K / 4)))) << 4) & 0x30303030;
    const int qh1 =
        (qh >> (2 * ((kqsx % (QI6_K / 2)) / (QI6_K / 4)))) & 0x30303030;

    const int kq0 = ky - ky % QI6_K + k % (QI6_K / 2) + 0;
    const int kq1 = ky - ky % QI6_K + k % (QI6_K / 2) + (QI6_K / 2);

    x_ql[i * (2 * WARP_SIZE + 1) + kq0] = dpct::vectorized_binary<sycl::char4>(
        ql0 | qh0, 0x20202020, dpct::sub_sat());
    x_ql[i * (2 * WARP_SIZE + 1) + kq1] = dpct::vectorized_binary<sycl::char4>(
        ql1 | qh1, 0x20202020, dpct::sub_sat());
  }
...
#pragma unroll
  for (int i0 = 0; i0 < mmq_y; i0 += nwarps * 8) {
    ...
    const block_q6_K *bxi =
        bx0 + i * blocks_per_row + (k % (WARP_SIZE / 8)) / 4;

    x_sc[i * (WARP_SIZE / 8) + i / 8 + k % (WARP_SIZE / 8)] =
        get_int_from_int8(bxi->scales, k % (QI6_K / 8));
  }
}
...
static __dpct_inline__ float vec_dot_iq2_xxs_q8_1(
  ...
  const uint8_t *aux8 = (const uint8_t *)q2;
  // printf("q2: %d, %d %d %d %d\n", iqs, q2[0], q2[1], q2[2], q2[3]);
  sycl::ext::oneapi::experimental::printf("q2: %d, %d %d %d %d\n", iqs, q2[0], q2[1], q2[2], q2[3]);
  const int8_t *q8 = bq8_1[ib32].qs;
  // sycl::ext::oneapi::experimental::printf("q8: %d, %d %d %d %d\n", iqs, q8[0], q8[1], q8[2], q8[3]);
...
*/
static __dpct_inline__ void mul_mat_q(
    const void *__restrict__ vx, const void *__restrict__ vy,
    float *__restrict__ dst, const int ncols_x, const int nrows_x,
    const int ncols_y, const int nrows_y, const int nrows_dst, int *tile_x_ql,
    sycl::half2 *tile_x_dm, int *tile_x_qh, int *tile_x_sc,
...
static void mul_mat_q4_0(const void *__restrict__ vx,
...
template <bool need_check>
static void mul_mat_q4_1(const void *__restrict__ vx,
                         const void *__restrict__ vy, float *__restrict__ dst,
                         const int ncols_x, const int nrows_x,
                         const int ncols_y, const int nrows_y,
                         const int nrows_dst, const sycl::nd_item<3> &item_ct1,
...
template <bool need_check>
static void mul_mat_q5_0(const void *__restrict__ vx,
                         const void *__restrict__ vy, float *__restrict__ dst,
                         const int ncols_x, const int nrows_x,
                         const int ncols_y, const int nrows_y,
                         const int nrows_dst, const sycl::nd_item<3> &item_ct1,
...
template <bool need_check>
static void mul_mat_q5_1(const void *__restrict__ vx,
                         const void *__restrict__ vy, float *__restrict__ dst,
                         const int ncols_x, const int nrows_x,
                         const int ncols_y, const int nrows_y,
                         const int nrows_dst, const sycl::nd_item<3> &item_ct1,
...
template <bool need_check>
static void mul_mat_q8_0(const void *__restrict__ vx,
                         const void *__restrict__ vy, float *__restrict__ dst,
                         const int ncols_x, const int nrows_x,
                         const int ncols_y, const int nrows_y,
                         const int nrows_dst, const sycl::nd_item<3> &item_ct1,
...
template <bool need_check>
static void mul_mat_q2_K(const void *__restrict__ vx,
                         const void *__restrict__ vy, float *__restrict__ dst,
                         const int ncols_x, const int nrows_x,
                         const int ncols_y, const int nrows_y,
                         const int nrows_dst, const sycl::nd_item<3> &item_ct1,
                         int *tile_x_ql_q2_K, sycl::half2 *tile_x_dm_q2_K,
                         int *tile_x_sc_q2_K, int *tile_y_qs,
...
template <bool need_check>
static void mul_mat_q3_K(const void *__restrict__ vx,
                         const void *__restrict__ vy, float *__restrict__ dst,
                         const int ncols_x, const int nrows_x,
                         const int ncols_y, const int nrows_y,
                         const int nrows_dst, const sycl::nd_item<3> &item_ct1,
...
template <bool need_check>
static void mul_mat_q5_K(const void *__restrict__ vx,
                         const void *__restrict__ vy, float *__restrict__ dst,
                         const int ncols_x, const int nrows_x,
                         const int ncols_y, const int nrows_y,
                         const int nrows_dst, const sycl::nd_item<3> &item_ct1,
                         int *tile_x_ql_q5_K, sycl::half2 *tile_x_dm_q5_K,
                         int *tile_x_sc_q5_K, int *tile_y_qs,
                         sycl::half2 *tile_y_ds) {
  int *tile_x_ql = nullptr;
  sycl::half2 *tile_x_dm = nullptr;
  int *tile_x_qh = nullptr;
...
template <bool need_check>
static void mul_mat_q6_K(const void *__restrict__ vx,
                         const void *__restrict__ vy, float *__restrict__ dst,
                         const int ncols_x, const int nrows_x,
                         const int ncols_y, const int nrows_y,
                         const int nrows_dst, const sycl::nd_item<3> &item_ct1,
...
static void dequantize_mul_mat_vec(const void *__restrict__ vx,
  ...
  const int vals_per_iter =
      iter_stride / WARP_SIZE;  // num quantized vals per thread and i iter
  const int y_offset = qr == 1 ? 1 : qk / 2;

// partial sum for each thread
#ifdef GGML_SYCL_F16
  sycl::half2 tmp = {
      0.0f, 0.0f};  // two sums for f16 to take advantage of half2 intrinsics
#else
  float tmp = 0.0f;
#endif  // GGML_SYCL_F16
...
static void mul_mat_p021_f16_f32(const void *__restrict__ vx,
  ...
  for (int col_x0 = 0; col_x0 < ncols_x;
    ...
    const float xi = sycl::vec<sycl::half, 1>(x[ix])
...
static void mul_mat_vec_nc_f16_f32(  // nc == non-contiguous
    ...
    const int channel_x_divisor, const sycl::nd_item<3> &item_ct1) {
  const sycl::half *x = (const sycl::half *)vx;
...
static void cpy_f32_f16(const char *cx, char *cdst, const int ne,
                        const int ne00, const int ne01, const int nb00,
                        const int nb01, const int nb02, const int ne10,
                        const int ne11, const int nb10, const int nb11,
                        const int nb12, const sycl::nd_item<3> &item_ct1) {
  const int i = item_ct1.get_local_range(2) * item_ct1.get_group(2) +
                item_ct1.get_local_id(2);
  ...
  // determine indices i02/i12, i01/i11, i00/i10 as a function of index i of
  // flattened tensor then combine those indices with the corresponding byte
  // offsets to get the total offsets
...
static void cpy_f32_q(const char *cx, char *cdst, const int ne, const int ne00,
                      const int ne01, const int nb00, const int nb01,
                      const int nb02, const int ne10, const int ne11,
...
// https://github.com/jquesnelle/yarn MIT licensed. Copyright (c) 2023 Jeffrey
// Quesnelle and Bowen Peng.
static void rope_yarn(float theta_extrap, float freq_scale,
                      rope_corr_dims corr_dims, int64_t i0, float ext_factor,
...
static void rope(const T *x, T *dst, int ncols, const int32_t *pos,
                 float freq_scale, int p_delta_rows, float freq_base,
  ...
  rope_yarn(theta_base, freq_scale, corr_dims, col, ext_factor, attn_factor,
            &cos_theta, &sin_theta);
...
template <typename T, bool has_pos>
static void rope_neox(const T *x, T *dst, int ncols, int n_dims,
                      ...
                      float inv_ndims, const sycl::nd_item<3> &item_ct1) {
  const int col = 2 * (item_ct1.get_local_range(1) * item_ct1.get_group(1) +
                       item_ct1.get_local_id(1));
  ...
  const int p = has_pos ? pos[i2] : 0;
  const float theta_base = p * freq_scale * dpct::pow(theta_scale, col / 2.0f);

  float cos_theta, sin_theta;
  rope_yarn(theta_base, freq_scale, corr_dims, cur_rot, ext_factor, attn_factor,
            &cos_theta, &sin_theta);
...
static void rope_glm_f32(const float *x, float *dst, int ncols,
                         const int32_t *pos, float freq_scale, int p_delta_rows,
                         float freq_base, int n_ctx,
                         const sycl::nd_item<3> &item_ct1) {
...
static void alibi_f32(const float *x, float *dst, const int ncols,
                      const int k_rows, const int n_heads_log2_floor,
...
static void diag_mask_inf_f32(const float *x, float *dst, const int ncols,
...
static void soft_max_f32(const float *x, const float *y, float *dst,
                         const int ncols, const int nrows_y, const float scale,
                         const sycl::nd_item<3> &item_ct1, float *buf) {
  const int tid = item_ct1.get_local_id(2);
  const int rowx = item_ct1.get_group(2);
  const int rowy =
      rowx % nrows_y;  // broadcast the mask (y) in the row dimension
...
static void scale_f32(const float *x, float *dst, const float scale,
                      const int k, const sycl::nd_item<3> &item_ct1) {
  const int i = item_ct1.get_local_range(2) * item_ct1.get_group(2) +
                item_ct1.get_local_id(2);
...
static void clamp_f32(const float *x, float *dst, const float min,
...
static void im2col_f32_f16(const float *x, sycl::half *dst, int offset_delta,
  ...
  const int64_t offset_dst = (item_ct1.get_group(1) * OW + ix) * CHW +
...
static void get_rows_sycl(const ggml_tensor *src0, const ggml_tensor *src1,
  ...
  const int block_num_x = (ne00 + 2 * SYCL_GET_ROWS_BLOCK_SIZE - 1) /
                          (2 * SYCL_GET_ROWS_BLOCK_SIZE);
  ...
  stream->parallel_for(sycl::nd_range<3>(block_nums * block_dims, block_dims),
                       [=](sycl::nd_item<3> item_ct1) {
                         k_get_rows<qk, qr, dq>(src0_dd, src1_dd, dst_dd, ne00,
                                                ne12, s1, s2, s3, nb01, nb02,
                                                nb03, s10, s11, s12, item_ct1);
                       });
...
static void get_rows_sycl_float(const ggml_tensor *src0,
  ...
  const int block_num_x =
      (ne00 + SYCL_GET_ROWS_BLOCK_SIZE - 1) / SYCL_GET_ROWS_BLOCK_SIZE;
  const sycl::range<3> block_nums(ne11 * ne12, ne10, block_num_x);
...

  {
    ...
    stream->parallel_for(sycl::nd_range<3>(block_nums * block_dims, block_dims),
                         [=](sycl::nd_item<3> item_ct1) {
                           k_get_rows_float(src0_dd, src1_dd, dst_dd, ne00,
                                            ne12, s1, s2, s3, nb01, nb02, nb03,
...
struct bin_bcast_sycl {
  ...
  void operator()(const struct ggml_tensor *src0,
    ...
    }
    {
      ...
      block_dims[1] =
          std::min<unsigned int>(ne1, block_size / (unsigned int)block_dims[2]);
      block_dims[0] =
          std::min(std::min<unsigned int>(
      ...
      if (block_nums[0] > 65535) {
        // this is the maximum number of blocks in z direction, fallback to 1D
        // grid kernel
          ...
          stream->parallel_for(
              ...
              [=](sycl::nd_item<3> item_ct1) {
                k_bin_bcast_unravel<bin_op>(
                    src0_dd, src1_dd, dst_dd, ne0, ne1, ne2, ne3, ne10, ne11,
        ...
        */
        ...
        stream->parallel_for(
            ...
            [=](sycl::nd_item<3> item_ct1) {
              k_bin_bcast<bin_op>(src0_dd, src1_dd, dst_dd, ne0, ne1, ne2, ne3,
...
static void gelu_f32_sycl(const float *x, float *dst, const int k,
  ...
  stream->parallel_for(
      sycl::nd_range<3>(sycl::range<3>(1, 1, num_blocks) *
                        ...
                        sycl::range<3>(1, 1, SYCL_GELU_BLOCK_SIZE)),
      [=](sycl::nd_item<3> item_ct1) { gelu_f32(x, dst, k, item_ct1); });
}
...
static void silu_f32_sycl(const float *x, float *dst, const int k,
  ...
  stream->parallel_for(
      sycl::nd_range<3>(sycl::range<3>(1, 1, num_blocks) *
                        ...
                        sycl::range<3>(1, 1, SYCL_SILU_BLOCK_SIZE)),
      [=](sycl::nd_item<3> item_ct1) { silu_f32(x, dst, k, item_ct1); });
}
...
static void gelu_quick_f32_sycl(const float *x, float *dst, const int k,
  ...
  stream->parallel_for(
      sycl::nd_range<3>(sycl::range<3>(1, 1, num_blocks) *
                            sycl::range<3>(1, 1, SYCL_GELU_BLOCK_SIZE),
                        sycl::range<3>(1, 1, SYCL_GELU_BLOCK_SIZE)),
      [=](sycl::nd_item<3> item_ct1) { gelu_quick_f32(x, dst, k, item_ct1); });
}
...
static void tanh_f32_sycl(const float *x, float *dst, const int k,
  ...
  stream->parallel_for(
      sycl::nd_range<3>(sycl::range<3>(1, 1, num_blocks) *
                        ...
                        sycl::range<3>(1, 1, SYCL_TANH_BLOCK_SIZE)),
      [=](sycl::nd_item<3> item_ct1) { tanh_f32(x, dst, k, item_ct1); });
}
...
static void relu_f32_sycl(const float *x, float *dst, const int k,
  ...
  stream->parallel_for(
      sycl::nd_range<3>(sycl::range<3>(1, 1, num_blocks) *
                        ...
                        sycl::range<3>(1, 1, SYCL_RELU_BLOCK_SIZE)),
      [=](sycl::nd_item<3> item_ct1) { relu_f32(x, dst, k, item_ct1); });
}
...
static void sqr_f32_sycl(const float *x, float *dst, const int k,
  ...
  stream->parallel_for(
      sycl::nd_range<3>(sycl::range<3>(1, 1, num_blocks) *
                        ...
                        sycl::range<3>(1, 1, SYCL_SQR_BLOCK_SIZE)),
      [=](sycl::nd_item<3> item_ct1) { sqr_f32(x, dst, k, item_ct1); });
}
...
static void norm_f32_sycl(const float *x, float *dst, const int ncols,
  ...
  if (ncols < 1024) {
    ...
    stream->submit([&](sycl::handler &cgh) {
      sycl::local_accessor<sycl::float2, 1> s_sum_acc_ct1(sycl::range<1>(32),
                                                          cgh);
...
static void upscale_f32_sycl(const float *x, float *dst, const int ne00,
  ...
  int ne0 = (ne00 * scale_factor);
  int num_blocks =
      (ne0 + SYCL_UPSCALE_BLOCK_SIZE - 1) / SYCL_UPSCALE_BLOCK_SIZE;
  sycl::range<3> gridDim(ne02, (ne01 * scale_factor), num_blocks);
  stream->parallel_for(
      sycl::nd_range<3>(gridDim * sycl::range<3>(1, 1, SYCL_UPSCALE_BLOCK_SIZE),
...
static void quantize_row_q8_1_sycl(const float *x, void *vy, const int kx,
                                   ...
                                   dpct::queue_ptr stream) {
  const int block_num_x =
      (kx_padded + SYCL_QUANTIZE_BLOCK_SIZE - 1) / SYCL_QUANTIZE_BLOCK_SIZE;
...
static void dequantize_block_sycl(const void *__restrict__ vx,
  ...
  const int num_blocks =
      (k + SYCL_DEQUANTIZE_BLOCK_SIZE - 1) / SYCL_DEQUANTIZE_BLOCK_SIZE;
  {
    dpct::has_capability_or_fail(stream->get_device(), {sycl::aspect::fp16});
...
static void dequantize_mul_mat_vec_q4_0_sycl(const void *vx, const dfloat *y,
  ...
  // the number of rows may exceed maximum grid size in the y or z dimensions,
  // use the x dimension instead
...
static void dequantize_mul_mat_vec_q2_K_sycl(const void *vx, const float *y,
  ...
  const int ny =
      2;  // very slightly faster than 1 even when K_QUANTS_PER_ITERATION = 2
  ...
  stream->parallel_for(
      ...
      [=](sycl::nd_item<3> item_ct1) [[intel::reqd_sub_group_size(32)]] {
        mul_mat_vec_q<QK4_0, QI4_0, block_q4_0, VDR_Q4_0_Q8_1_MMVQ,
                      vec_dot_q4_0_q8_1>(vx, vy, dst, ncols, nrows, item_ct1);
      });
}
  ...
  stream->parallel_for(
      ...
      [=](sycl::nd_item<3> item_ct1) [[intel::reqd_sub_group_size(32)]] {
        mul_mat_vec_q<QK4_0, QI4_1, block_q4_1, VDR_Q4_1_Q8_1_MMVQ,
                      vec_dot_q4_1_q8_1>(vx, vy, dst, ncols, nrows, item_ct1);
      });
}
  ...
  stream->parallel_for(
      ...
      [=](sycl::nd_item<3> item_ct1) [[intel::reqd_sub_group_size(32)]] {
        mul_mat_vec_q<QK5_0, QI5_0, block_q5_0, VDR_Q5_0_Q8_1_MMVQ,
                      vec_dot_q5_0_q8_1>(vx, vy, dst, ncols, nrows, item_ct1);
      });
}
  ...
  stream->parallel_for(
      ...
      [=](sycl::nd_item<3> item_ct1) [[intel::reqd_sub_group_size(32)]] {
        mul_mat_vec_q<QK5_1, QI5_1, block_q5_1, VDR_Q5_1_Q8_1_MMVQ,
                      vec_dot_q5_1_q8_1>(vx, vy, dst, ncols, nrows, item_ct1);
      });
}
  ...
  stream->parallel_for(
      ...
      [=](sycl::nd_item<3> item_ct1) [[intel::reqd_sub_group_size(32)]] {
        mul_mat_vec_q<QK8_0, QI8_0, block_q8_0, VDR_Q8_0_Q8_1_MMVQ,
                      vec_dot_q8_0_q8_1>(vx, vy, dst, ncols, nrows, item_ct1);
      });
}
  ...
  stream->parallel_for(
      ...
      [=](sycl::nd_item<3> item_ct1) [[intel::reqd_sub_group_size(32)]] {
        mul_mat_vec_q<QK_K, QI2_K, block_q2_K, VDR_Q2_K_Q8_1_MMVQ,
                      vec_dot_q2_K_q8_1>(vx, vy, dst, ncols, nrows, item_ct1);
      });
}
  ...
  stream->parallel_for(
      ...
      [=](sycl::nd_item<3> item_ct1) [[intel::reqd_sub_group_size(32)]] {
        mul_mat_vec_q<QK_K, QI3_K, block_q3_K, VDR_Q3_K_Q8_1_MMVQ,
                      vec_dot_q3_K_q8_1>(vx, vy, dst, ncols, nrows, item_ct1);
      });
}
  ...
  stream->parallel_for(
      ...
      [=](sycl::nd_item<3> item_ct1) [[intel::reqd_sub_group_size(32)]] {
        mul_mat_vec_q<QK_K, QI4_K, block_q4_K, VDR_Q4_K_Q8_1_MMVQ,
                      vec_dot_q4_K_q8_1>(vx, vy, dst, ncols, nrows, item_ct1);
      });
}
  ...
  stream->parallel_for(
      ...
      [=](sycl::nd_item<3> item_ct1) [[intel::reqd_sub_group_size(32)]] {
        mul_mat_vec_q<QK_K, QI5_K, block_q5_K, VDR_Q5_K_Q8_1_MMVQ,
                      vec_dot_q5_K_q8_1>(vx, vy, dst, ncols, nrows, item_ct1);
      });
}
  ...
  stream->parallel_for(
      ...
      [=](sycl::nd_item<3> item_ct1) [[intel::reqd_sub_group_size(32)]] {
        mul_mat_vec_q<QK_K, QI6_K, block_q6_K, VDR_Q6_K_Q8_1_MMVQ,
                      vec_dot_q6_K_q8_1>(vx, vy, dst, ncols, nrows, item_ct1);
      });
}
...
static void ggml_mul_mat_q4_0_q8_1_sycl(const void *vx, const void *vy,
  ...
  int id;
  SYCL_CHECK(CHECK_TRY_ERROR(id = get_current_device_index()));
    ...
    */
    {
      ...
      stream->submit([&](sycl::handler &cgh) {
        ...
        sycl::local_accessor<float, 1> tile_x_d_q4_0_acc_ct1(
            sycl::range<1>(mmq_y * (WARP_SIZE / QI4_0) + mmq_y / QI4_0), cgh);
    ...
    */
    {
      ...
      stream->submit([&](sycl::handler &cgh) {
        ...
        sycl::local_accessor<sycl::half2, 1> tile_x_dm_q4_1_acc_ct1(
            sycl::range<1>(mmq_y * (WARP_SIZE / QI4_1) + mmq_y / QI4_1), cgh);
        sycl::local_accessor<int, 1> tile_y_qs_acc_ct1(
            sycl::range<1>(mmq_x * WARP_SIZE), cgh);
        sycl::local_accessor<sycl::half2, 1> tile_y_ds_acc_ct1(
            sycl::range<1>(mmq_x * WARP_SIZE / QI8_1), cgh);
  ...
  const int num_blocks = (ne + SYCL_CPY_BLOCK_SIZE - 1) / SYCL_CPY_BLOCK_SIZE;
  {
    ...
    stream->parallel_for(
        sycl::nd_range<3>(sycl::range<3>(1, 1, num_blocks) *
                          ...
                          sycl::range<3>(1, 1, SYCL_CPY_BLOCK_SIZE)),
        [=](sycl::nd_item<3> item_ct1) {
          cpy_f32_f16<cpy_1_f32_f32>(cx, cdst, ne, ne00, ne01, nb00, nb01, nb02,
  ...
  const int num_blocks = (ne + SYCL_CPY_BLOCK_SIZE - 1) / SYCL_CPY_BLOCK_SIZE;
  {
    ...
    stream->parallel_for(
        sycl::nd_range<3>(sycl::range<3>(1, 1, num_blocks) *
                          ...
                          sycl::range<3>(1, 1, SYCL_CPY_BLOCK_SIZE)),
        [=](sycl::nd_item<3> item_ct1) {
          cpy_f32_f16<cpy_1_f32_f16>(cx, cdst, ne, ne00, ne01, nb00, nb01, nb02,
                                     ne10, ne11, nb10, nb11, nb12, item_ct1);
        });
  }
}
  ...
  stream->parallel_for(sycl::nd_range<3>(sycl::range<3>(1, 1, num_blocks),
                       ...
                       [=](sycl::nd_item<3> item_ct1) {
                         cpy_f32_q<cpy_blck_f32_q8_0, QK8_0>(
                             cx, cdst, ne, ne00, ne01, nb00, nb01, nb02, ne10,
                             ne11, nb10, nb11, nb12, item_ct1);
                       });
}
  ...
  const int num_blocks = (ne + SYCL_CPY_BLOCK_SIZE - 1) / SYCL_CPY_BLOCK_SIZE;
  {
    ...
    stream->parallel_for(
        sycl::nd_range<3>(sycl::range<3>(1, 1, num_blocks) *
                          ...
                          sycl::range<3>(1, 1, SYCL_CPY_BLOCK_SIZE)),
        [=](sycl::nd_item<3> item_ct1) {
          cpy_f32_f16<cpy_1_f16_f16>(cx, cdst, ne, ne00, ne01, nb00, nb01, nb02,
                                     ne10, ne11, nb10, nb11, nb12, item_ct1);
        });
  }
}
  ...
  const int num_blocks = (ne + SYCL_CPY_BLOCK_SIZE - 1) / SYCL_CPY_BLOCK_SIZE;
  {
    ...
    stream->parallel_for(
        sycl::nd_range<3>(sycl::range<3>(1, 1, num_blocks) *
                          ...
                          sycl::range<3>(1, 1, SYCL_CPY_BLOCK_SIZE)),
        [=](sycl::nd_item<3> item_ct1) {
          cpy_f32_f16<cpy_1_i16_i16>(cx, cdst, ne, ne00, ne01, nb00, nb01, nb02,
                                     ne10, ne11, nb10, nb11, nb12, item_ct1);
        });
  }
}
  ...
  const int num_blocks = (ne + SYCL_CPY_BLOCK_SIZE - 1) / SYCL_CPY_BLOCK_SIZE;
  {
    ...
    stream->parallel_for(
        sycl::nd_range<3>(sycl::range<3>(1, 1, num_blocks) *
                          ...
                          sycl::range<3>(1, 1, SYCL_CPY_BLOCK_SIZE)),
        [=](sycl::nd_item<3> item_ct1) {
          cpy_f32_f16<cpy_1_i32_i32>(cx, cdst, ne, ne00, ne01, nb00, nb01, nb02,
                                     ne10, ne11, nb10, nb11, nb12, item_ct1);
        });
  }
}
...
static void scale_f32_sycl(const float *x, float *dst, const float scale,
  ...
  const int num_blocks =
      (k + SYCL_SCALE_BLOCK_SIZE - 1) / SYCL_SCALE_BLOCK_SIZE;
  stream->parallel_for(
      sycl::nd_range<3>(sycl::range<3>(1, 1, num_blocks) *
...
static void clamp_f32_sycl(const float *x, float *dst, const float min,
                           ...
                           dpct::queue_ptr stream) {
  const int num_blocks =
      (k + SYCL_CLAMP_BLOCK_SIZE - 1) / SYCL_CLAMP_BLOCK_SIZE;
  stream->parallel_for(
      sycl::nd_range<3>(sycl::range<3>(1, 1, num_blocks) *
...
static void rope_sycl(const T *x, T *dst, int ncols, int nrows,
  ...
  const int num_blocks_x =
      (ncols + 2 * SYCL_ROPE_BLOCK_SIZE - 1) / (2 * SYCL_ROPE_BLOCK_SIZE);
  ...
  if (pos == nullptr) {
    ...
    */
    ...
    stream->parallel_for(sycl::nd_range<3>(block_nums * block_dims, block_dims),
                         [=](sycl::nd_item<3> item_ct1) {
                           rope<T, false>(x, dst, ncols, pos, freq_scale,
                                          p_delta_rows, freq_base, ext_factor,
                                          attn_factor, corr_dims, item_ct1);
                         });
  } else {
    /*
    ...
    */
    ...
    stream->parallel_for(sycl::nd_range<3>(block_nums * block_dims, block_dims),
                         [=](sycl::nd_item<3> item_ct1) {
                           rope<T, true>(x, dst, ncols, pos, freq_scale,
                                         p_delta_rows, freq_base, ext_factor,
                                         attn_factor, corr_dims, item_ct1);
                         });
  }
  ...
  if (pos == nullptr) {
    ...
    */
    ...
    stream->parallel_for(sycl::nd_range<3>(block_nums * block_dims, block_dims),
                         [=](sycl::nd_item<3> item_ct1) {
                           rope_neox<T, false>(
                               x, dst, ncols, n_dims, pos, freq_scale,
                               p_delta_rows, ext_factor, attn_factor, corr_dims,
                               theta_scale, inv_ndims, item_ct1);
                         });
  } else {
    /*
...
static void rope_glm_f32_sycl(const float *x, float *dst, int ncols, int nrows,
  ...
  const int num_blocks_x =
      (ncols + SYCL_ROPE_BLOCK_SIZE - 1) / SYCL_ROPE_BLOCK_SIZE;
  const sycl::range<3> block_nums(1, nrows, num_blocks_x);
  stream->parallel_for(sycl::nd_range<3>(block_nums * block_dims, block_dims),
                       [=](sycl::nd_item<3> item_ct1) {
                         rope_glm_f32(x, dst, ncols, pos, freq_scale,
                                      p_delta_rows, freq_base, n_ctx, item_ct1);
                       });
}
...
static void alibi_f32_sycl(const float *x, float *dst, const int ncols,
  ...
  const int num_blocks_x =
      (ncols + SYCL_ALIBI_BLOCK_SIZE - 1) / (SYCL_ALIBI_BLOCK_SIZE);
  const sycl::range<3> block_nums(1, nrows, num_blocks_x);
  stream->parallel_for(sycl::nd_range<3>(block_nums * block_dims, block_dims),
                       [=](sycl::nd_item<3> item_ct1) {
                         alibi_f32(x, dst, ncols, k_rows, n_heads_log2_floor,
                                   m0, m1, item_ct1);
                       });
}
...
static void argsort_f32_i32_sycl(const float *x, int *dst, const int ncols,
  ...
  if (order == GGML_SORT_ASC) {
    ...
    */
    stream->parallel_for(sycl::nd_range<3>(block_nums * block_dims, block_dims),
                         [=](sycl::nd_item<3> item_ct1) {
                           k_argsort_f32_i32<GGML_SORT_ASC>(x, dst, ncols,
                                                            item_ct1);
                         });
    ...
    */
    stream->parallel_for(sycl::nd_range<3>(block_nums * block_dims, block_dims),
                         [=](sycl::nd_item<3> item_ct1) {
                           k_argsort_f32_i32<GGML_SORT_DESC>(x, dst, ncols,
                                                             item_ct1);
                         });
  } else {
    GGML_ASSERT(false);
...
static void diag_mask_inf_f32_sycl(const float *x, float *dst,
  ...
  const int block_num_x = (ncols_x + SYCL_DIAG_MASK_INF_BLOCK_SIZE - 1) /
                          SYCL_DIAG_MASK_INF_BLOCK_SIZE;
  const sycl::range<3> block_nums(1, block_num_x, nrows_x);
  stream->parallel_for(sycl::nd_range<3>(block_nums * block_dims, block_dims),
                       [=](sycl::nd_item<3> item_ct1) {
                         diag_mask_inf_f32(x, dst, ncols_x, rows_per_channel,
                                           n_past, item_ct1);
                       });
}
  ...
  stream->submit([&](sycl::handler &cgh) {
    ...
    */
    ...
    cgh.parallel_for(sycl::nd_range<3>(block_nums * block_dims, block_dims),
                     [=](sycl::nd_item<3> item_ct1)
                         [[intel::reqd_sub_group_size(32)]] {
                           soft_max_f32(x, y, dst, ncols_x, nrows_y, scale,
                                        item_ct1, buf_acc_ct1.get_pointer());
                         });
  });
}
...
static void im2col_f32_f16_sycl(const float *x, sycl::half *dst, int IW, int IH,
  ...
  const int num_blocks =
      (parallel_elements + SYCL_IM2COL_BLOCK_SIZE - 1) / SYCL_IM2COL_BLOCK_SIZE;
  sycl::range<3> block_nums(IC, OH, num_blocks);
  {
    ...
    stream->parallel_for(
        sycl::nd_range<3>(
            block_nums * sycl::range<3>(1, 1, SYCL_IM2COL_BLOCK_SIZE),
            sycl::range<3>(1, 1, SYCL_IM2COL_BLOCK_SIZE)),
        [=](sycl::nd_item<3> item_ct1) {
          im2col_f32_f16(x, dst, offset_delta, IW, IH, OW, KW, KH,
                         parallel_elements, (IC * KH * KW), s0, s1, p0, p1, d0,
                         d1, item_ct1);
        });
  }
}
...
struct scoped_spin_lock {
  ...
  ~scoped_spin_lock() { lock.clear(std::memory_order_release); }
...
static void *ggml_sycl_pool_malloc_leg(size_t size, size_t *actual_size) try {
  ...
  SYCL_CHECK(CHECK_TRY_ERROR(
      ptr = (void *)sycl::malloc_device(look_ahead_size, *stream)));
  *actual_size = look_ahead_size;
  g_sycl_pool_size[id] += look_ahead_size;

#ifdef DEBUG_SYCL_MALLOC
  fprintf(stderr,
          "%s[%d]: %d buffers, max_size = %u MB, pool_size = %u MB, requested "
          "%u MB\n",
          __func__, id, nnz, (uint32_t)(max_size / 1024 / 1024),
          (uint32_t)(g_sycl_pool_size[id] / 1024 / 1024),
...
static void ggml_sycl_pool_free_leg(void *ptr, size_t size) try {
  ...
  fprintf(stderr,
          "WARNING: sycl buffer pool full, increase MAX_SYCL_BUFFERS\n");
...
struct sycl_pool_alloc {
  ...
  T *alloc(size_t size) {
    ...
    // sizeof(T), ptr, this->actual_size);
    return ptr;
  }

  sycl_pool_alloc(size_t size) { alloc(size); }
...
bool ggml_sycl_loaded(void) { return g_sycl_loaded; }
...
void ggml_backend_sycl_print_sycl_devices() {
  ...
  for (int id = 0; id < device_count; ++id) {
    ...
    fprintf(stderr,
            "  Device %d: %s,\tcompute capability %d.%d,\n\tmax compute_units "
            "%d,\tmax work group size %d,\tmax sub group size %d,\tglobal mem "
            "size %lu\n",
            id, prop.get_name(), prop.get_major_version(),
            prop.get_minor_version(), prop.get_max_compute_units(),
  ...
  if (!initialized) {
    ...
    for (int id = 0; id < g_all_sycl_device_count; ++id) {
      ...
      for (int is = 0; is < MAX_STREAMS; ++is) {
        ...
        */
        SYCL_CHECK(
            CHECK_TRY_ERROR(g_syclStreams[device_inx][is] =
                                dpct::get_current_device().create_queue()));
      }

      const dpct::queue_ptr stream = g_syclStreams[device_inx][0];
      // create sycl handle
      SYCL_CHECK(CHECK_TRY_ERROR(g_sycl_handles[device_inx] = stream));
      /*
    ...
    // for (int id = 0; id < g_all_sycl_device_count; ++id) {
    //     GGML_SYCL_DEBUG("id=%d  g_device_caps[%d].device_id=%d
    //     g_sycl_device_id2index[%d].index=%d ", id, id,
...
static dpct::err0 ggml_sycl_cpy_tensor_2d(void *dst,
  ...
  if (nb0 == ts && nb1 == ts * ne0 / bs) {
    ...
    // x, i1_diff * nb1); return CHECK_TRY_ERROR(stream->memcpy(dst_ptr, x,
    // i1_diff * nb1));
    return CHECK_TRY_ERROR(
        dpct::async_dpct_memcpy(dst_ptr, x, i1_diff * nb1, kind, *stream));
...
static void ggml_sycl_op_get_rows(const ggml_tensor *src0,
  ...
  switch (src0->type) {
    case GGML_TYPE_F16:
      get_rows_sycl_float(src0, src1, dst, (const sycl::half *)src0_d, src1_i32,
                          dst_d, stream);
      break;
    case GGML_TYPE_F32:
      ...
      break;
    case GGML_TYPE_Q4_0:
      get_rows_sycl<QK4_0, QR4_0, dequantize_q4_0>(src0, src1, dst, src0_d,
                                                   src1_i32, dst_d, stream);
      break;
    case GGML_TYPE_Q4_1:
      get_rows_sycl<QK4_1, QR4_1, dequantize_q4_1>(src0, src1, dst, src0_d,
                                                   src1_i32, dst_d, stream);
      break;
    case GGML_TYPE_Q5_0:
      get_rows_sycl<QK5_0, QR5_0, dequantize_q5_0>(src0, src1, dst, src0_d,
                                                   src1_i32, dst_d, stream);
      break;
    case GGML_TYPE_Q5_1:
      get_rows_sycl<QK5_1, QR5_1, dequantize_q5_1>(src0, src1, dst, src0_d,
                                                   src1_i32, dst_d, stream);
      break;
    case GGML_TYPE_Q8_0:
      get_rows_sycl<QK8_0, QR8_0, dequantize_q8_0>(src0, src1, dst, src0_d,
                                                   src1_i32, dst_d, stream);
      break;
    default:
      // TODO: k-quants
...
inline void ggml_sycl_op_bin_bcast(const ggml_tensor *src0,
  ...
  if (src0->type == GGML_TYPE_F32 && dst->type == GGML_TYPE_F32) {
    ...
    op()(src0, src1, dst, (const int32_t *)src0_dd, (const int32_t *)src1_dd,
         (int32_t *)dst_dd, main_stream);
  } else if (src0->type == GGML_TYPE_I16 && dst->type == GGML_TYPE_I16) {
    op()(src0, src1, dst, (const int16_t *)src0_dd, (const int16_t *)src1_dd,
         (int16_t *)dst_dd, main_stream);
  } else {
    fprintf(stderr, "%s: unsupported types: dst: %s, src0: %s, src1: %s\n",
            __func__, ggml_type_name(dst->type), ggml_type_name(src0->type),
...
static void ggml_sycl_op_repeat(const ggml_tensor *src0,
                                ...
                                float *dst_d,
                                const dpct::queue_ptr &main_stream) {
  ggml_sycl_op_bin_bcast<bin_bcast_sycl<op_repeat>>(dst, src0, dst, nullptr,
                                                    src0_d, dst_d, main_stream);
...
inline void ggml_sycl_op_add(const ggml_tensor *src0, const ggml_tensor *src1,
                             ...
                             const float *src1_dd, float *dst_dd,
                             const dpct::queue_ptr &main_stream) {
  ggml_sycl_op_bin_bcast<bin_bcast_sycl<op_add>>(src0, src1, dst, src0_dd,
                                                 src1_dd, dst_dd, main_stream);
}
...
inline void ggml_sycl_op_acc(const ggml_tensor *src0, const ggml_tensor *src1,
  ...
  acc_f32_sycl(src0_dd, src1_dd, dst_dd, ggml_nelements(dst), src1->ne[0],
               src1->ne[1], src1->ne[2], nb1, nb2, offset, main_stream);
...
inline void ggml_sycl_op_mul(const ggml_tensor *src0, const ggml_tensor *src1,
                             ...
                             const float *src1_dd, float *dst_dd,
                             const dpct::queue_ptr &main_stream) {
  ggml_sycl_op_bin_bcast<bin_bcast_sycl<op_mul>>(src0, src1, dst, src0_dd,
                                                 src1_dd, dst_dd, main_stream);
}
...
inline void ggml_sycl_op_div(const ggml_tensor *src0, const ggml_tensor *src1,
                             ...
                             const float *src1_dd, float *dst_dd,
                             const dpct::queue_ptr &main_stream) {
  ggml_sycl_op_bin_bcast<bin_bcast_sycl<op_div>>(src0, src1, dst, src0_dd,
                                                 src1_dd, dst_dd, main_stream);
}
...
inline void ggml_sycl_op_leaky_relu(const ggml_tensor *src0,
  ...
  leaky_relu_f32_sycl(src0_dd, dst_dd, ggml_nelements(src0), negative_slope,
                      main_stream);
...
inline void ggml_sycl_op_group_norm(const ggml_tensor *src0,
  ...
  int group_size =
      src0->ne[0] * src0->ne[1] * ((src0->ne[2] + num_groups - 1) / num_groups);
  group_norm_f32_sycl(src0_dd, dst_dd, num_groups, group_size,
                      src0->ne[0] * src0->ne[1] * src0->ne[2], main_stream);
...
inline void ggml_sycl_op_concat(const ggml_tensor *src0,
  ...
  for (int i3 = 0; i3 < dst->ne[3]; i3++) {
    concat_f32_sycl(src0_dd + i3 * (src0->nb[3] / 4),
                    src1_dd + i3 * (src1->nb[3] / 4),
                    dst_dd + i3 * (dst->nb[3] / 4), dst->ne[0], dst->ne[1],
                    dst->ne[2], src0->ne[2], main_stream);
  }
...
inline void ggml_sycl_op_upscale(const ggml_tensor *src0,
  ...
  upscale_f32_sycl(src0_dd, dst_dd, src0->ne[0], src0->ne[1], src0->ne[2],
                   scale_factor, main_stream);
...
inline void ggml_sycl_op_pad(const ggml_tensor *src0, const ggml_tensor *src1,
  ...
  pad_f32_sycl(src0_dd, dst_dd, src0->ne[0], src0->ne[1], src0->ne[2],
...
inline void ggml_sycl_op_mul_mat_q(
  ...
  // the main device has a larger memory buffer to hold the results from all
  // GPUs nrows_dst == nrows of the matrix that the dequantize_mul_mat kernel
  // writes into
  const int64_t nrows_dst =
      dst->backend == GGML_BACKEND_GPU && device_id == g_main_device ? ne0
                                                                     : row_diff;

  switch (src0->type) {
    case GGML_TYPE_Q4_0:
      ggml_mul_mat_q4_0_q8_1_sycl(src0_dd_i, src1_ddq_i, dst_dd_i, ne00,
                                  row_diff, src1_ncols, src1_padded_row_size,
                                  nrows_dst, stream);
      break;
    case GGML_TYPE_Q4_1:
      ggml_mul_mat_q4_1_q8_1_sycl(src0_dd_i, src1_ddq_i, dst_dd_i, ne00,
                                  row_diff, src1_ncols, src1_padded_row_size,
                                  nrows_dst, stream);
      break;
    case GGML_TYPE_Q5_0:
      ggml_mul_mat_q5_0_q8_1_sycl(src0_dd_i, src1_ddq_i, dst_dd_i, ne00,
                                  row_diff, src1_ncols, src1_padded_row_size,
                                  nrows_dst, stream);
      break;
    case GGML_TYPE_Q5_1:
      ggml_mul_mat_q5_1_q8_1_sycl(src0_dd_i, src1_ddq_i, dst_dd_i, ne00,
                                  row_diff, src1_ncols, src1_padded_row_size,
                                  nrows_dst, stream);
      break;
    case GGML_TYPE_Q8_0:
      ggml_mul_mat_q8_0_q8_1_sycl(src0_dd_i, src1_ddq_i, dst_dd_i, ne00,
                                  row_diff, src1_ncols, src1_padded_row_size,
                                  nrows_dst, stream);
      break;
    case GGML_TYPE_Q2_K:
      ggml_mul_mat_q2_K_q8_1_sycl(src0_dd_i, src1_ddq_i, dst_dd_i, ne00,
                                  row_diff, src1_ncols, src1_padded_row_size,
                                  nrows_dst, stream);
      break;
    case GGML_TYPE_Q3_K:
      ggml_mul_mat_q3_K_q8_1_sycl(src0_dd_i, src1_ddq_i, dst_dd_i, ne00,
                                  row_diff, src1_ncols, src1_padded_row_size,
                                  nrows_dst, stream);
      break;
    case GGML_TYPE_Q4_K:
      ...
      break;
    case GGML_TYPE_Q5_K:
      ggml_mul_mat_q5_K_q8_1_sycl(src0_dd_i, src1_ddq_i, dst_dd_i, ne00,
                                  row_diff, src1_ncols, src1_padded_row_size,
                                  nrows_dst, stream);
      break;
    case GGML_TYPE_Q6_K:
      ggml_mul_mat_q6_K_q8_1_sycl(src0_dd_i, src1_ddq_i, dst_dd_i, ne00,
                                  row_diff, src1_ncols, src1_padded_row_size,
                                  nrows_dst, stream);
      break;
    default:
...
static int64_t get_row_rounding(ggml_type type) {
  ...
  for (int64_t id = 0; id < g_device_count; ++id) {
    if (g_tensor_split[id] <
        (id + 1 < g_device_count ? g_tensor_split[id + 1] : 1.0f)) {
...
inline void ggml_sycl_op_mul_mat_vec_q(
  ...
  switch (src0->type) {
    case GGML_TYPE_Q4_0:
      mul_mat_vec_q4_0_q8_1_sycl(src0_dd_i, src1_ddq_i, dst_dd_i, ne00,
                                 row_diff, stream);
      break;
    case GGML_TYPE_Q4_1:
      mul_mat_vec_q4_1_q8_1_sycl(src0_dd_i, src1_ddq_i, dst_dd_i, ne00,
                                 row_diff, stream);
      break;
    case GGML_TYPE_Q5_0:
      mul_mat_vec_q5_0_q8_1_sycl(src0_dd_i, src1_ddq_i, dst_dd_i, ne00,
                                 row_diff, stream);
      break;
    case GGML_TYPE_Q5_1:
      mul_mat_vec_q5_1_q8_1_sycl(src0_dd_i, src1_ddq_i, dst_dd_i, ne00,
                                 row_diff, stream);
      break;
    case GGML_TYPE_Q8_0:
      mul_mat_vec_q8_0_q8_1_sycl(src0_dd_i, src1_ddq_i, dst_dd_i, ne00,
                                 row_diff, stream);
      break;
    case GGML_TYPE_Q2_K:
      mul_mat_vec_q2_K_q8_1_sycl(src0_dd_i, src1_ddq_i, dst_dd_i, ne00,
                                 row_diff, stream);
      break;
    case GGML_TYPE_Q3_K:
      mul_mat_vec_q3_K_q8_1_sycl(src0_dd_i, src1_ddq_i, dst_dd_i, ne00,
                                 row_diff, stream);
      break;
    case GGML_TYPE_Q4_K:
      ...
      break;
    case GGML_TYPE_Q5_K:
      mul_mat_vec_q5_K_q8_1_sycl(src0_dd_i, src1_ddq_i, dst_dd_i, ne00,
                                 row_diff, stream);
      break;
    case GGML_TYPE_Q6_K:
      mul_mat_vec_q6_K_q8_1_sycl(src0_dd_i, src1_ddq_i, dst_dd_i, ne00,
                                 row_diff, stream);
      break;
    case GGML_TYPE_IQ2_XXS:
...
inline void ggml_sycl_op_dequantize_mul_mat_vec(
  ...
  // on some GPUs it is faster to convert src1 to half and to use half precision
  // intrinsics
#ifdef GGML_SYCL_F16
  ...
  if (src1_convert_f16) {
    if (src1->type == GGML_TYPE_F16) {
      ...
      ggml_cpy_f32_f16_sycl((const char *)src1_ddf_i, (char *)src1_dfloat, ne00,
                            ne00, 1, sizeof(float), 0, 0, ne00, 1,
                            sizeof(sycl::half), 0, 0, stream);
    }
  }
#else
  const dfloat *src1_dfloat =
      (const dfloat *)src1_ddf_i;  // dfloat == float, no conversion
#endif  // GGML_SYCL_F16
  ...
  switch (src0->type) {
    case GGML_TYPE_Q4_0:
      ...
      break;
    case GGML_TYPE_Q4_1:
      dequantize_mul_mat_vec_q4_1_sycl(src0_dd_i, src1_dfloat, dst_dd_i, ne00,
                                       row_diff, stream);
      break;
    case GGML_TYPE_Q5_0:
      dequantize_mul_mat_vec_q5_0_sycl(src0_dd_i, src1_dfloat, dst_dd_i, ne00,
                                       row_diff, stream);
      break;
    case GGML_TYPE_Q5_1:
      ...
      break;
    case GGML_TYPE_Q8_0:
      dequantize_mul_mat_vec_q8_0_sycl(src0_dd_i, src1_dfloat, dst_dd_i, ne00,
                                       row_diff, stream);
      break;
    case GGML_TYPE_Q2_K:
      ...
      break;
    case GGML_TYPE_Q3_K:
      dequantize_mul_mat_vec_q3_K_sycl(src0_dd_i, src1_ddf_i, dst_dd_i, ne00,
                                       row_diff, stream);
      break;
    case GGML_TYPE_Q4_K:
      dequantize_mul_mat_vec_q4_K_sycl(src0_dd_i, src1_ddf_i, dst_dd_i, ne00,
                                       row_diff, stream);
      break;
    case GGML_TYPE_Q5_K:
      dequantize_mul_mat_vec_q5_K_sycl(src0_dd_i, src1_ddf_i, dst_dd_i, ne00,
                                       row_diff, stream);
      break;
    case GGML_TYPE_Q6_K:
      dequantize_mul_mat_vec_q6_K_sycl(src0_dd_i, src1_ddf_i, dst_dd_i, ne00,
                                       row_diff, stream);
      break;
    case GGML_TYPE_F16:
...
inline void ggml_sycl_op_mul_mat_sycl(
  ...
  // the main device has a larger memory buffer to hold the results from all
  // GPUs ldc == nrows of the matrix that cuBLAS writes into
  int ldc = dst->backend == GGML_BACKEND_GPU && device_id == g_main_device
                ? ne0
                : row_diff;
  ...
  if ((src0->type == GGML_TYPE_F16 || ggml_is_quantized(src0->type)) &&
    ...
    SYCL_CHECK(CHECK_TRY_ERROR(
        dpct::gemm(*g_sycl_handles[id], oneapi::mkl::transpose::trans,
    ...
    const float *src0_ddf_i = src0->type == GGML_TYPE_F32
                                  ? (const float *)src0_dd_i
                                  : src0_ddq_as_f32.get();
    ...
    SYCL_CHECK(CHECK_TRY_ERROR(oneapi::mkl::blas::column_major::gemm(
        *g_sycl_handles[id], oneapi::mkl::transpose::trans,
        ...
        src1_ddf_i, ne10, dpct::get_value(&beta, *g_sycl_handles[id]), dst_dd_i,
        ldc)));
  }
...
inline void ggml_sycl_op_rope(const ggml_tensor *src0, const ggml_tensor *src1,
  ...
  ggml_rope_yarn_corr_dims(n_dims, n_orig_ctx, freq_base, beta_fast, beta_slow,
                           corr_dims.v);

  // compute
  if (is_glm) {
    GGML_ASSERT(false);
    rope_glm_f32_sycl(src0_dd, dst_dd, ne00, nrows, pos, freq_scale, ne01,
                      freq_base, n_ctx, main_stream);
  } else if (is_neox) {
    if (src0->type == GGML_TYPE_F32) {
      rope_neox_sycl((const float *)src0_dd, (float *)dst_dd, ne00, n_dims,
                     nrows, pos, freq_scale, ne01, freq_base, ext_factor,
                     attn_factor, corr_dims, main_stream);
    } else if (src0->type == GGML_TYPE_F16) {
      rope_neox_sycl((const sycl::half *)src0_dd, (sycl::half *)dst_dd, ne00,
                     n_dims, nrows, pos, freq_scale, ne01, freq_base,
                     ext_factor, attn_factor, corr_dims, main_stream);
    } else {
      GGML_ASSERT(false);
    }
  } else {
    if (src0->type == GGML_TYPE_F32) {
      rope_sycl((const float *)src0_dd, (float *)dst_dd, ne00, nrows, pos,
                freq_scale, ne01, freq_base, ext_factor, attn_factor, corr_dims,
                main_stream);
    } else if (src0->type == GGML_TYPE_F16) {
      rope_sycl((const sycl::half *)src0_dd, (sycl::half *)dst_dd, ne00, nrows,
                pos, freq_scale, ne01, freq_base, ext_factor, attn_factor,
                corr_dims, main_stream);
    } else {
      GGML_ASSERT(false);
    }
  }
...
inline void ggml_sycl_op_alibi(const ggml_tensor *src0, const ggml_tensor *src1,
  ...
  alibi_f32_sycl(src0_dd, dst_dd, ne00, nrows, ne01, n_heads_log2_floor, m0, m1,
                 main_stream);
...
inline void ggml_sycl_op_im2col(const ggml_tensor *src0,
  ...
  im2col_f32_f16_sycl(src1_dd, (sycl::half *)dst_dd, IW, IH, OW, OH, KW, KH, IC,
                      delta_offset, s0, s1, p0, p1, d0, d1, main_stream);
...
inline void ggml_sycl_op_diag_mask_inf(const ggml_tensor *src0,
  ...
  diag_mask_inf_f32_sycl(src0_dd, dst_dd, ne00, nrows0, ne01, n_past,
                         main_stream);
...
inline void ggml_sycl_op_soft_max(const ggml_tensor *src0,
  ...
  GGML_ASSERT(!src1 ||
              src1->type ==
                  GGML_TYPE_F32);  // src1 contains mask and it is optional
  ...
  soft_max_f32_sycl(src0_dd, src1 ? src1_dd : nullptr, dst_dd, ne00, nrows_x,
                    nrows_y, scale, main_stream);
...
static void ggml_sycl_op_flatten(const ggml_tensor *src0,
  ...
  ggml_tensor_extra_gpu *src1_extra =
      use_src1 ? (ggml_tensor_extra_gpu *)src1->extra : nullptr;
  ...
  ggml_sycl_set_device(g_main_device);
  dpct::queue_ptr main_stream = g_syclStreams[g_main_device_index][0];
  // GGML_SYCL_DEBUG("g_main_device_index=%d, main_stream=%p src0_on_device=%d,
  // src1_on_device=%d, dst_on_device=%d\n", g_main_device_index, main_stream,
  // src0_on_device, src1_on_device, dst_on_device);
  ...
  if (src0_on_device) {
    ...
    // src0_ddf, src0);
    SYCL_CHECK(
        ggml_sycl_cpy_tensor_2d(src0_ddf, src0, 0, 0, 0, nrows0, main_stream));
  }
  ...
  if (use_src1) {
    if (src1_on_device) {
      ...
      SYCL_CHECK(ggml_sycl_cpy_tensor_2d(src1_ddf, src1, 0, 0, 0, nrows1,
                                         main_stream));
    }
  }
  if (dst_on_device) {
  ...
  // GGML_SYCL_DEBUG("op src0=%p, src1=%p, dst=%p, src0_ddf=%p, src1_ddf=%p,
  // dst_ddf=%p, main_stream=%p\n", src0, src1, dst, src0_ddf, src1_ddf,
  // dst_ddf, main_stream);
  // do the computation
...
static void ggml_sycl_op_mul_mat(const ggml_tensor *src0,
  ...
  for (int64_t id = 0; id < g_device_count; ++id) {
    ...
    const bool src1_on_device =
        src1->backend == GGML_BACKEND_GPU && id == g_main_device_index;
    const bool dst_on_device =
        dst->backend == GGML_BACKEND_GPU && id == g_main_device_index;
    ...
    if (src0_on_device && src0_is_contiguous) {
      ...
      // const size_t size_src0_ddq = split ? (row_high[id]-row_low[id])*ne00 *
      // src0_ts/src0_bs : ggml_nbytes(src0);
      src0_dd[id] =
          (char *)ggml_sycl_pool_malloc(ggml_nbytes(src0), &src0_as[id]);
    }
    ...
    if (src1_on_device && src1_is_contiguous) {
      ...
      src1_ddf[id] =
          (float *)ggml_sycl_pool_malloc(ggml_nbytes(src1), &src1_asf[id]);
    }

    if (convert_src1_to_q8_1) {
      src1_ddq[id] = (char *)ggml_sycl_pool_malloc(
          nrows1 * src1_padded_col_size * q8_1_ts / q8_1_bs, &src1_asq[id]);

      if (src1_on_device && src1_is_contiguous) {
        quantize_row_q8_1_sycl(src1_ddf[id], src1_ddq[id], ne10, nrows1,
    ...
    if (dst_on_device) {
      ...
      const size_t size_dst_ddf =
          split ? (row_high[id] - row_low[id]) * ne1 * sizeof(float)
                : ggml_nbytes(dst);
  ...
  // here an event is recorded that signals that the main device has finished
  ...
  if (split && used_devices > 1) {
    ...
    */
    SYCL_CHECK(CHECK_TRY_ERROR(*src0_extra->events[g_main_device_index][0] =
                                   g_syclStreams[g_main_device_index][0]
                                       ->ext_oneapi_submit_barrier()));
  }

  const int64_t src1_col_stride =
      split && used_devices > 1 ? MUL_MAT_SRC1_COL_STRIDE : ne11;
  for (int64_t src1_col_0 = 0; src1_col_0 < ne11;
       src1_col_0 += src1_col_stride) {
    const int64_t is = split ? (src1_col_0 / src1_col_stride) % MAX_STREAMS : 0;
    const int64_t src1_ncols = src1_col_0 + src1_col_stride > ne11
                                   ? ne11 - src1_col_0
                                   : src1_col_stride;

    for (int64_t id = 0; id < g_device_count; ++id) {
      if ((!split && id != g_main_device_index) ||
      ...
      for (int64_t i0 = 0; i0 < ne13 * ne12; ++i0) {
        ...
        const size_t src1_ddq_i_offset =
            (i0 * ne11 + src1_col_0) * src1_padded_col_size * q8_1_ts / q8_1_bs;

        // for split tensors the data begins at i0 == i0_offset_low
        char *src0_dd_i = src0_dd[id] + (i0 / i02_divisor) *
                                            (ne01 * ne00 * src0_ts) / src0_bs;
        float *src1_ddf_i = src1_ddf[id] + (i0 * ne11 + src1_col_0) * ne10;
        char *src1_ddq_i = src1_ddq[id] + src1_ddq_i_offset;
        float *dst_dd_i = dst_dd[id] + (i0 * ne1 + src1_col_0) *
                                           (dst_on_device ? ne0 : row_diff);

        // the main device memory buffer can be on VRAM scratch, with space for
        // all partial results in that case an offset on dst_ddf_i is needed
          ...
          if (id != g_main_device_index) {
            if (convert_src1_to_q8_1) {
              char *src1_ddq_i_source =
                  src1_ddq[g_main_device_index] + src1_ddq_i_offset;
              SYCL_CHECK(CHECK_TRY_ERROR(stream->memcpy(
                  src1_ddq_i, src1_ddq_i_source,
                  src1_ncols * src1_padded_col_size * q8_1_ts / q8_1_bs)));
            } else {
              float *src1_ddf_i_source =
                  (float *)src1_extra->data_device[g_main_device_index];
              src1_ddf_i_source += (i0 * ne11 + src1_col_0) * ne10;
              SYCL_CHECK(CHECK_TRY_ERROR(
                  stream->memcpy(src1_ddf_i, src1_ddf_i_source,
                                 src1_ncols * ne10 * sizeof(float))));
            }
          }
        } else if (src1->backend == GGML_BACKEND_CPU ||
                   (src1_on_device && !src1_is_contiguous)) {
          SYCL_CHECK(ggml_sycl_cpy_tensor_2d(src1_ddf_i, src1, i03, i02,
                                             src1_col_0,
        ...
        if (convert_src1_to_q8_1 &&
            (src1->backend == GGML_BACKEND_CPU || !src1_is_contiguous)) {
          quantize_row_q8_1_sycl(src1_ddf_i, src1_ddq_i, ne10, src1_ncols,
                                 src1_padded_col_size, stream);
          /*
        ...
        if (src1_col_0 == 0 && (!src0_on_device || !src0_is_contiguous) &&
            i02 % i02_divisor == 0) {
          SYCL_CHECK(ggml_sycl_cpy_tensor_2d(src0_dd_i, src0, i03,
                                             i02 / i02_divisor, row_low[id],
                                             row_high[id], stream));
        }
        if (src1->type == GGML_TYPE_F16) {
        ...
        */
        ...
        if (!dst_on_device) {
          ...
          if (split) {
            // src0 = weight matrix is saved as a transposed matrix for better
            // memory layout. dst is NOT transposed. The outputs of matrix
            // matrix multiplications can therefore NOT simply be concatenated
            // for >1 GPU. Instead they need to be copied to the correct slice
            // in ne0 = dst row index. If dst is a vector with ne0 == 1 then you
            // don't have to do this but it still produces correct results.
            float *dhf_dst_i =
                (float *)((char *)dst_off_device + i02 * nb2 + i03 * nb3);
            GGML_ASSERT(dst->nb[1] == ne0 * sizeof(float));
            dhf_dst_i += src1_col_0 * ne0 + row_low[id];
            SYCL_CHECK(CHECK_TRY_ERROR(dpct::async_dpct_memcpy(
                dhf_dst_i, ne0 * sizeof(float), dst_dd_i,
                row_diff * sizeof(float), row_diff * sizeof(float), src1_ncols,
                kind, *stream)));
          } else {
            float *dhf_dst_i =
                (float *)((char *)dst_off_device + i02 * nb2 + i03 * nb3);
            ...
            SYCL_CHECK(CHECK_TRY_ERROR(stream->memcpy(
                dhf_dst_i, dst_dd_i, src1_ncols * ne0 * sizeof(float))));
          }
        }
        ...
        if (split && (id != g_main_device_index || is != 0)) {
          ...
          */
          SYCL_CHECK(CHECK_TRY_ERROR(*src0_extra->events[id][is] =
  ...
  if (split && g_device_count > 1) {
    int64_t is_max =
        (ne11 + MUL_MAT_SRC1_COL_STRIDE - 1) / MUL_MAT_SRC1_COL_STRIDE;
...
static void ggml_sycl_repeat(const ggml_tensor *src0, const ggml_tensor *src1,
                             ggml_tensor *dst) {
  GGML_SYCL_DEBUG("call %s\n", __func__);
  ggml_sycl_op_flatten(src0, src1, dst, ggml_sycl_op_repeat);
}

static void ggml_sycl_get_rows(const ggml_tensor *src0, const ggml_tensor *src1,
                               ggml_tensor *dst) {
  GGML_SYCL_DEBUG("call %s\n", __func__);
  ggml_sycl_op_flatten(src0, src1, dst, ggml_sycl_op_get_rows);
}

static void ggml_sycl_add(const ggml_tensor *src0, const ggml_tensor *src1,
                          ggml_tensor *dst) {
  GGML_SYCL_DEBUG("call %s\n", __func__);
  ggml_sycl_op_flatten(src0, src1, dst, ggml_sycl_op_add);
  // log_tensor_with_cnt("log_ggml_sycl_add_src0", (struct ggml_tensor *) src0,
  // 6); log_tensor_with_cnt("log_ggml_sycl_add_src1", (struct ggml_tensor
  // *)src1, 6); log_tensor_with_cnt("log_ggml_sycl_add_dst", dst, 6);
}

static void ggml_sycl_acc(const ggml_tensor *src0, const ggml_tensor *src1,
                          ggml_tensor *dst) {
  GGML_SYCL_DEBUG("call %s\n", __func__);
  ggml_sycl_op_flatten(src0, src1, dst, ggml_sycl_op_acc);
}

static void ggml_sycl_mul(const ggml_tensor *src0, const ggml_tensor *src1,
                          ggml_tensor *dst) {
  GGML_SYCL_DEBUG("call %s\n", __func__);
  ggml_sycl_op_flatten(src0, src1, dst, ggml_sycl_op_mul);
  // log_tensor_with_cnt("log_ggml_sycl_mul_src0", (struct ggml_tensor *)src0,
  // 6); log_tensor_with_cnt("log_ggml_sycl_mul_src1", (struct ggml_tensor
  // *)src1, 6); log_tensor_with_cnt("log_ggml_sycl_mul_dst", dst, 6);
}

static void ggml_sycl_div(const ggml_tensor *src0, const ggml_tensor *src1,
                          ggml_tensor *dst) {
  GGML_SYCL_DEBUG("call %s\n", __func__);
  ggml_sycl_op_flatten(src0, src1, dst, ggml_sycl_op_div);
}

static void ggml_sycl_gelu(const ggml_tensor *src0, const ggml_tensor *src1,
                           ggml_tensor *dst) {
  GGML_SYCL_DEBUG("call %s\n", __func__);
  ggml_sycl_op_flatten(src0, src1, dst, ggml_sycl_op_gelu);
}

static void ggml_sycl_silu(const ggml_tensor *src0, const ggml_tensor *src1,
                           ggml_tensor *dst) {
  GGML_SYCL_DEBUG("call %s\n", __func__);
  ggml_sycl_op_flatten(src0, src1, dst, ggml_sycl_op_silu);
}

static void ggml_sycl_gelu_quick(const ggml_tensor *src0,
                                 const ggml_tensor *src1, ggml_tensor *dst) {
  GGML_SYCL_DEBUG("call %s\n", __func__);
  ggml_sycl_op_flatten(src0, src1, dst, ggml_sycl_op_gelu_quick);
}

static void ggml_sycl_tanh(const ggml_tensor *src0, const ggml_tensor *src1,
                           ggml_tensor *dst) {
  GGML_SYCL_DEBUG("call %s\n", __func__);
  ggml_sycl_op_flatten(src0, src1, dst, ggml_sycl_op_tanh);
}

static void ggml_sycl_relu(const ggml_tensor *src0, const ggml_tensor *src1,
                           ggml_tensor *dst) {
  GGML_SYCL_DEBUG("call %s\n", __func__);
  ggml_sycl_op_flatten(src0, src1, dst, ggml_sycl_op_relu);
}

static void ggml_sycl_leaky_relu(const ggml_tensor *src0,
                                 const ggml_tensor *src1, ggml_tensor *dst) {
  GGML_SYCL_DEBUG("call %s\n", __func__);
  ggml_sycl_op_flatten(src0, src1, dst, ggml_sycl_op_leaky_relu);
}

static void ggml_sycl_sqr(const ggml_tensor *src0, const ggml_tensor *src1,
                          ggml_tensor *dst) {
  GGML_SYCL_DEBUG("call %s\n", __func__);
  ggml_sycl_op_flatten(src0, src1, dst, ggml_sycl_op_sqr);
}

static void ggml_sycl_norm(const ggml_tensor *src0, const ggml_tensor *src1,
                           ggml_tensor *dst) {
  GGML_SYCL_DEBUG("call %s\n", __func__);
  ggml_sycl_op_flatten(src0, src1, dst, ggml_sycl_op_norm);
}

static void ggml_sycl_group_norm(const ggml_tensor *src0,
                                 const ggml_tensor *src1, ggml_tensor *dst) {
  GGML_SYCL_DEBUG("call %s\n", __func__);
  ggml_sycl_op_flatten(src0, src1, dst, ggml_sycl_op_group_norm);
}

static void ggml_sycl_concat(const ggml_tensor *src0, const ggml_tensor *src1,
                             ggml_tensor *dst) {
  GGML_SYCL_DEBUG("call %s\n", __func__);
  ggml_sycl_op_flatten(src0, src1, dst, ggml_sycl_op_concat);
}

static void ggml_sycl_upscale(const ggml_tensor *src0, const ggml_tensor *src1,
                              ggml_tensor *dst) {
  GGML_SYCL_DEBUG("call %s\n", __func__);
  ggml_sycl_op_flatten(src0, src1, dst, ggml_sycl_op_upscale);
}

static void ggml_sycl_pad(const ggml_tensor *src0, const ggml_tensor *src1,
                          ggml_tensor *dst) {
  GGML_SYCL_DEBUG("call %s\n", __func__);
  ggml_sycl_op_flatten(src0, src1, dst, ggml_sycl_op_pad);
}

static void ggml_sycl_rms_norm(const ggml_tensor *src0, const ggml_tensor *src1,
                               ggml_tensor *dst) {
  GGML_SYCL_DEBUG("call %s\n", __func__);
  ggml_sycl_op_flatten(src0, src1, dst, ggml_sycl_op_rms_norm);
  // log_tensor_with_cnt("log_ggml_sycl_rms_norm_src0", (struct ggml_tensor
  // *)src0, 6); log_tensor_with_cnt("log_ggml_sycl_rms_norm_src1", (struct
  // ggml_tensor *)src1, 6); log_tensor_with_cnt("log_ggml_sycl_rms_norm_dst",
  // dst, 6);
}
...
static void ggml_sycl_mul_mat_vec_p021(const ggml_tensor *src0,
  ...
  GGML_ASSERT(src0->nb[0] <= src0->nb[1] &&
              src0->nb[2] <= src0->nb[3]);  // 0213 permutation
  GGML_ASSERT(src1->nb[0] <= src1->nb[1] &&
              src1->nb[2] <= src1->nb[3]);  // 0213 permutation
  GGML_ASSERT(src0->type == GGML_TYPE_F16);
  GGML_ASSERT(src1->type == GGML_TYPE_F32);
  ...
  ggml_mul_mat_p021_f16_f32_sycl(src0_ddq, src1_ddf, dst_ddf, ne00, ne01, ne02,
                                 ne12, main_stream);
} catch (sycl::exception const &exc) {
  std::cerr << exc.what() << "Exception caught at file:" << __FILE__
            << ", line:" << __LINE__ << std::endl;
...
static void ggml_sycl_mul_mat_vec_nc(const ggml_tensor *src0,
  ...
  ggml_mul_mat_vec_nc_f16_f32_sycl(src0_ddq, src1_ddf, dst_ddf, ne00, ne01,
                                   row_stride_x, ne02, ne12, channel_stride_x,
                                   main_stream);
} catch (sycl::exception const &exc) {
  std::cerr << exc.what() << "Exception caught at file:" << __FILE__
            << ", line:" << __LINE__ << std::endl;
...
static void k_compute_batched_ptrs(const sycl::half *src0_as_f16,
  ...
  ptrs_src[0 * ne23 + i12 + i13 * ne12] =
      (const char *)src0_as_f16 + i02 * nb02 + i03 * nb03;
  ptrs_src[1 * ne23 + i12 + i13 * ne12] =
      (const char *)src1_as_f16 + i12 * nb12 / 2 + i13 * nb13 / 2;
...
#else
  if (r2 == 1 && r3 == 1 && src0->nb[2] * src0->ne[2] == src0->nb[3] &&
      src1->nb[2] * src1->ne[2] == src1->nb[3]) {
    ...
    SYCL_CHECK(CHECK_TRY_ERROR(dpct::gemm_batch(
        *g_sycl_handles[g_main_device_index], oneapi::mkl::transpose::trans,
        ...
        nb11 / sizeof(float), src1->nb[2] / sizeof(float), beta, (char *)dst_t,
        cu_data_type, ne01, dst->nb[2] / sizeof(float), ne12 * ne13,
        cu_compute_type)));
  } else {
        ...
        cgh.parallel_for(sycl::nd_range<3>(block_dims, block_dims),
                         [=](sycl::nd_item<3> item_ct1) {
                           k_compute_batched_ptrs(
                               src0_as_f16, src1_as_f16_get_ct1, dst_t,
                               ptrs_src_get_ct3, ptrs_dst_get_ct4, ne12, ne13,
                               ne23, nb02, nb03, nb12, nb13, nbd2, nbd3, r2, r3,
                               item_ct1);
                         });
      });
    }
...
static void ggml_sycl_mul_mat(const ggml_tensor *src0, const ggml_tensor *src1,
                              ggml_tensor *dst) {
  const bool all_on_device = (src0->backend == GGML_BACKEND_GPU ||
                              src0->backend == GGML_BACKEND_GPU_SPLIT) &&
  ...
  // src0->ne[3]); printf("      %8d %8d %8d %8d\n", src0->nb[0], src0->nb[1],
  // src0->nb[2], src0->nb[3]); printf("src1: %8d %8d %8d %8d\n", src1->ne[0],
  // src1->ne[1], src1->ne[2], src1->ne[3]); printf("      %8d %8d %8d %8d\n",
  // src1->nb[0], src1->nb[1], src1->nb[2], src1->nb[3]); printf("src0 is
  // contiguous %d, transposed %d, type = %s, name = %s\n",
  // ggml_is_contiguous(src0), ggml_is_transposed(src0),
  // ggml_type_name(src0->type), src0->name); printf("src1 is contiguous %d,
  // transposed %d, type = %s, name = %s\n", ggml_is_contiguous(src1),
  // ggml_is_transposed(src1), ggml_type_name(src1->type), src1->name);

  if (!split && all_on_device && !use_xmx && src0->type == GGML_TYPE_F16 &&
      ggml_is_permuted(src0) && ggml_is_permuted(src1) && src1->ne[1] == 1) {
    // KQ single-batch
    // GGML_SYCL_DEBUG("ggml_sycl_mul_mat_vec_p021\n");
    ggml_sycl_mul_mat_vec_p021(src0, src1, dst);
  } else if (!split && all_on_device && !use_xmx &&
             src0->type == GGML_TYPE_F16 && !ggml_is_contiguous(src0) &&
             !ggml_is_transposed(src1) && src1->ne[1] == 1) {
    // KQV single-batch
    ...
    if (src1->ne[1] == 1 && src0->ne[0] % GGML_SYCL_DMMV_X == 0) {
...
#else
      const bool use_mul_mat_vec_q = min_compute_capability >= VER_4VEC &&
                                     ggml_is_quantized(src0->type) &&
                                     ggml_nrows(src1) == 1;
#endif  // GGML_SYCL_FORCE_DMMV
      if (use_mul_mat_vec_q) {
        // NOTE: this kernel does not support ggml_nrows(src1) > 1
        // GGML_SYCL_DEBUG("ggml_sycl_mul_mat ggml_sycl_op_mul_mat_vec_q
        // path\n");
        ...
        ggml_sycl_op_mul_mat(src0, src1, dst,
                             ggml_sycl_op_dequantize_mul_mat_vec, false);
      }
    } else {
      ...
      if (use_mul_mat_q) {
        ...
        // GGML_SYCL_DEBUG("ggml_sycl_mul_mat ggml_sycl_op_mul_mat_sycl
        // path\n");
        ggml_sycl_op_mul_mat(src0, src1, dst, ggml_sycl_op_mul_mat_sycl, false);
      }
    }
...
static void ggml_sycl_mul_mat_id(const ggml_tensor *src0,
  ...
  if (ids->backend == GGML_BACKEND_GPU) {
    const char *ids_dev =
        (const char *)((const ggml_tensor_extra_gpu *)ids->extra)
            ->data_device[g_main_device_index];
    SYCL_CHECK(CHECK_TRY_ERROR(
  ...
  char *src1_original =
      src1->backend == GGML_BACKEND_CPU
          ? (char *)src1->data
          : (char *)src1_extra->data_device[g_main_device_index];
  char *dst_original =
      dst->backend == GGML_BACKEND_CPU
          ? (char *)dst->data
          : (char *)dst_extra->data_device[g_main_device_index];
  ...
  if (src1->ne[1] == 1) {
    ...
    for (int64_t i01 = 0; i01 < ids->ne[1]; i01++) {
      // int32_t row_id;
      // SYCL_CHECK(syclMemcpyAsync(&row_id, ids_dev + i01*ids->nb[1] +
      // id*ids->nb[0], sizeof(int32_t), syclMemcpyDeviceToHost,
      // g_syclStreams[g_main_device][0]));
      ...
      src1_row_extra.data_device[g_main_device_index] =
          src1_original + i01 * src1->nb[1];
      src1_row.data =
          (char *)src1->data + i01 * src1->nb[1];  // TODO why is this set?

      dst_row_extra.data_device[g_main_device_index] =
          dst_original + i01 * dst->nb[1];
      dst_row.data =
          (char *)dst->data + i01 * dst->nb[1];  // TODO why is this set?
    ...
    const dpct::memcpy_direction src1_kind = src1->backend == GGML_BACKEND_CPU
                                                 ? dpct::host_to_device
    ...
    for (int32_t row_id = 0; row_id < n_as; ++row_id) {
      ...
      for (int64_t i01 = 0; i01 < ids->ne[1]; i01++) {
        ...
        SYCL_CHECK(CHECK_TRY_ERROR(
            stream->memcpy(dst_original + i01 * nb1,
...
static void ggml_sycl_scale(const ggml_tensor *src0, const ggml_tensor *src1,
                            ggml_tensor *dst) {
  ggml_sycl_op_flatten(src0, src1, dst, ggml_sycl_op_scale);
}

static void ggml_sycl_clamp(const ggml_tensor *src0, const ggml_tensor *src1,
                            ggml_tensor *dst) {
...
static void ggml_sycl_cpy(const ggml_tensor *src0, const ggml_tensor *src1,
  ...
  const ggml_tensor_extra_gpu *src1_extra =
      (ggml_tensor_extra_gpu *)src1->extra;
  ...
  if (src0->type == GGML_TYPE_F32 && src1->type == GGML_TYPE_F32) {
    ggml_cpy_f32_f32_sycl(src0_ddc, src1_ddc, ne, ne00, ne01, nb00, nb01, nb02,
                          ne10, ne11, nb10, nb11, nb12, main_stream);
  } else if (src0->type == GGML_TYPE_F32 && src1->type == GGML_TYPE_F16) {
    ggml_cpy_f32_f16_sycl(src0_ddc, src1_ddc, ne, ne00, ne01, nb00, nb01, nb02,
                          ne10, ne11, nb10, nb11, nb12, main_stream);
  } else if (src0->type == GGML_TYPE_F32 && src1->type == GGML_TYPE_Q8_0) {
    ggml_cpy_f32_q8_0_sycl(src0_ddc, src1_ddc, ne, ne00, ne01, nb00, nb01, nb02,
                           ne10, ne11, nb10, nb11, nb12, main_stream);
  } else if (src0->type == GGML_TYPE_F32 && src1->type == GGML_TYPE_Q4_0) {
    ggml_cpy_f32_q4_0_sycl(src0_ddc, src1_ddc, ne, ne00, ne01, nb00, nb01, nb02,
                           ne10, ne11, nb10, nb11, nb12, main_stream);
  } else if (src0->type == GGML_TYPE_F32 && src1->type == GGML_TYPE_Q4_1) {
    ggml_cpy_f32_q4_1_sycl(src0_ddc, src1_ddc, ne, ne00, ne01, nb00, nb01, nb02,
                           ne10, ne11, nb10, nb11, nb12, main_stream);
  } else if (src0->type == GGML_TYPE_F16 && src1->type == GGML_TYPE_F16) {
    ggml_cpy_f16_f16_sycl(src0_ddc, src1_ddc, ne, ne00, ne01, nb00, nb01, nb02,
                          ne10, ne11, nb10, nb11, nb12, main_stream);
  } else if (src0->type == GGML_TYPE_I16 && src1->type == GGML_TYPE_I16) {
    ggml_cpy_i16_i16_sycl(src0_ddc, src1_ddc, ne, ne00, ne01, nb00, nb01, nb02,
                          ne10, ne11, nb10, nb11, nb12, main_stream);
  } else if (src0->type == GGML_TYPE_I32 && src1->type == GGML_TYPE_I32) {
    ggml_cpy_i32_i32_sycl(src0_ddc, src1_ddc, ne, ne00, ne01, nb00, nb01, nb02,
                          ne10, ne11, nb10, nb11, nb12, main_stream);
  } else {
...
static void ggml_sycl_dup(const ggml_tensor *src0, const ggml_tensor *src1,
                          ggml_tensor *dst) {
...
static void ggml_sycl_diag_mask_inf(const ggml_tensor *src0,
                                    const ggml_tensor *src1, ggml_tensor *dst) {
  ggml_sycl_op_flatten(src0, src1, dst, ggml_sycl_op_diag_mask_inf);
}

static void ggml_sycl_soft_max(const ggml_tensor *src0, const ggml_tensor *src1,
                               ggml_tensor *dst) {
  ggml_sycl_op_flatten(src0, src1, dst, ggml_sycl_op_soft_max);
}

static void ggml_sycl_rope(const ggml_tensor *src0, const ggml_tensor *src1,
                           ggml_tensor *dst) {
  GGML_ASSERT(
      ggml_is_contiguous(src0));  // TODO: this restriction is temporary until
                                  // non-cont support is implemented
  ggml_sycl_op_flatten(src0, src1, dst, ggml_sycl_op_rope);
}

static void ggml_sycl_alibi(const ggml_tensor *src0, const ggml_tensor *src1,
                            ggml_tensor *dst) {
  ggml_sycl_op_flatten(src0, src1, dst, ggml_sycl_op_alibi);
}

static void ggml_sycl_im2col(const ggml_tensor *src0, const ggml_tensor *src1,
                             ggml_tensor *dst) {
  ggml_sycl_op_flatten(src0, src1, dst, ggml_sycl_op_im2col);
}

static void ggml_sycl_sum_rows(const ggml_tensor *src0, const ggml_tensor *src1,
                               ggml_tensor *dst) {
  GGML_ASSERT(ggml_is_contiguous(src0));
  ggml_sycl_op_flatten(src0, src1, dst, ggml_sycl_op_sum_rows);
}

static void ggml_sycl_argsort(const ggml_tensor *src0, const ggml_tensor *src1,
                              ggml_tensor *dst) {
  GGML_ASSERT(ggml_is_contiguous(src0));
  ggml_sycl_op_flatten(src0, src1, dst, ggml_sycl_op_argsort);
}

static void ggml_sycl_nop(const ggml_tensor *src0, const ggml_tensor *src1,
                          ggml_tensor *dst) {
  (void)src0;
  (void)src1;
  (void)dst;
...
static size_t ggml_nbytes_split(const struct ggml_tensor *tensor,
                                int nrows_split) {
...
void ggml_sycl_transform_tensor(void *data, struct ggml_tensor *tensor) try {
  ...
  for (int64_t id = 0; id < g_device_count; ++id) {
    ...
    // pad last row to a multiple of 512 elements to avoid out-of-bounds memory
    // accesses
    ...
    char *buf;
    SYCL_CHECK(
        CHECK_TRY_ERROR(buf = (char *)sycl::malloc_device(size, *stream)));
    ...
    SYCL_CHECK(
        CHECK_TRY_ERROR((*stream).memcpy(buf, buf_host, original_size).wait()));

    extra->data_device[id] = buf;

    if (backend == GGML_BACKEND_GPU_SPLIT) {
      for (int64_t is = 0; is < MAX_STREAMS; ++is) {
        SYCL_CHECK(CHECK_TRY_ERROR(extra->events[id][is] = new sycl::event()));
      }
    }
  }
  ...
  for (int64_t id = 0; id < g_device_count; ++id) {
    ...
    for (int64_t is = 0; is < MAX_STREAMS; ++is) {
      if (extra->events[id][is] != nullptr) {
        SYCL_CHECK(ggml_sycl_set_device(get_device_id_by_index(id)));
        SYCL_CHECK(CHECK_TRY_ERROR(dpct::destroy_event(extra->events[id][is])));
      }
    }
  }
...
static ggml_tensor_extra_gpu *ggml_sycl_alloc_temp_tensor_extra() {
  ...
  g_temp_tensor_extra_index =
      (g_temp_tensor_extra_index + 1) % GGML_SYCL_MAX_NODES;
...
static void ggml_sycl_assign_buffers_impl(struct ggml_tensor *tensor,
  ...
  if (tensor->src[0] != nullptr &&
      tensor->src[0]->backend == GGML_BACKEND_CPU) {
    const ggml_op src0_op = tensor->src[0]->op;
    if (src0_op == GGML_OP_RESHAPE || src0_op == GGML_OP_TRANSPOSE ||
        src0_op == GGML_OP_VIEW || src0_op == GGML_OP_PERMUTE) {
      ggml_sycl_assign_buffers_impl(tensor->src[0], scratch, force_inplace,
                                    no_alloc);
    }
  }
  if (tensor->op == GGML_OP_CPY &&
      tensor->src[1]->backend == GGML_BACKEND_CPU) {
  ...
  const bool inplace =
      (tensor->src[0] != nullptr && tensor->src[0]->data == tensor->data) ||
      tensor->op == GGML_OP_VIEW || force_inplace;
  const size_t size = ggml_nbytes(tensor);

  SYCL_CHECK(ggml_sycl_set_device(g_main_device));
  const dpct::queue_ptr stream = g_syclStreams[g_main_device_index][0];

  if (inplace && (tensor->src[0]->backend == GGML_BACKEND_GPU ||
                  tensor->src[0]->backend == GGML_BACKEND_GPU_SPLIT)) {
    ggml_tensor_extra_gpu *src0_extra =
        (ggml_tensor_extra_gpu *)tensor->src[0]->extra;
    char *src0_ddc = (char *)src0_extra->data_device[g_main_device_index];
    size_t offset = 0;
    ...
    ggml_tensor_extra_gpu *src1_extra =
        (ggml_tensor_extra_gpu *)tensor->src[1]->extra;
    ...
    if (data == nullptr) {
      SYCL_CHECK(CHECK_TRY_ERROR(
          data = (char *)sycl::malloc_device(g_scratch_size, *stream)));
    ...
    void *data;
    SYCL_CHECK(
        CHECK_TRY_ERROR(data = (void *)sycl::malloc_device(size, *stream)));
    SYCL_CHECK(CHECK_TRY_ERROR((*stream).memset(data, 0, size).wait()));
...
void ggml_sycl_assign_scratch_offset(struct ggml_tensor *tensor,
  ...
  if (g_scratch_buffer == nullptr) {
    ggml_sycl_set_device(g_main_device);
    const dpct::queue_ptr stream = g_syclStreams[g_main_device_index][0];
    SYCL_CHECK(CHECK_TRY_ERROR(g_scratch_buffer = (void *)sycl::malloc_device(
  ...
  if (inplace && (tensor->view_src->backend == GGML_BACKEND_GPU ||
                  tensor->view_src->backend == GGML_BACKEND_GPU_SPLIT)) {
    ggml_tensor_extra_gpu *src0_extra =
        (ggml_tensor_extra_gpu *)tensor->view_src->extra;
    char *src0_ddc = (char *)src0_extra->data_device[g_main_device_index];
...
void ggml_sycl_set_main_device(const int main_device) try {
  if (main_device >= g_all_sycl_device_count) {
    fprintf(stderr,
            "warning: cannot set main_device=%d because there are only %d "
            "devices. Using device %d instead.\n",
...
void ggml_sycl_set_scratch_size(const size_t scratch_size) {
  // this is a hack to not completely break llama.cpp when using multiple models
  // or contexts simultaneously it still won't always work as expected, but it's
  // better than nothing
...
void ggml_sycl_free_scratch() try {
  ...
  SYCL_CHECK(CHECK_TRY_ERROR(sycl::free(g_scratch_buffer, *stream)));
...
bool ggml_sycl_compute_forward(struct ggml_compute_params *params,
                               struct ggml_tensor *tensor) {
  if (!g_sycl_loaded) return false;

  ggml_sycl_func_t func;
  const bool any_on_device =
      tensor->backend == GGML_BACKEND_GPU ||
      (tensor->src[0] != nullptr &&
       (tensor->src[0]->backend == GGML_BACKEND_GPU ||
        tensor->src[0]->backend == GGML_BACKEND_GPU_SPLIT)) ||
      (tensor->src[1] != nullptr &&
       tensor->src[1]->backend == GGML_BACKEND_GPU);
...
#ifndef NDEBUG
      fprintf(stderr,
              "%s: cannot compute %s: src0->ne[3] = %" PRId64
              ", src1->ne[3] = %" PRId64 " - fallback to CPU\n",
              __func__, tensor->name, tensor->src[0]->ne[3],
              tensor->src[1]->ne[3]);
#endif
      return false;
    }
  ...
  switch (tensor->op) {
    ...
    case GGML_OP_MUL_MAT:
      if (!any_on_device &&
          !ggml_sycl_can_mul_mat(tensor->src[0], tensor->src[1], tensor)) {
        return false;
      }
    ...
    case GGML_OP_MUL_MAT_ID:
      if (!any_on_device &&
          !ggml_sycl_can_mul_mat(tensor->src[2], tensor->src[1], tensor)) {
        return false;
      }
...
struct ggml_backend_sycl_buffer_context {
  ...
  ~ggml_backend_sycl_buffer_context() { delete[] temp_tensor_extras; }
  ...
  ggml_tensor_extra_gpu *ggml_sycl_alloc_temp_tensor_extra() {
    ...
    temp_tensor_extra_index =
        (temp_tensor_extra_index + 1) % GGML_SYCL_MAX_NODES;
...
GGML_CALL static const char *ggml_backend_sycl_buffer_get_name(
...
static void ggml_backend_sycl_buffer_free_buffer(
  ...
  SYCL_CHECK(CHECK_TRY_ERROR(sycl::free(ctx->dev_ptr, *stream)));
  delete ctx;
} catch (sycl::exception const &exc) {
  std::cerr << exc.what() << "Exception caught at file:" << __FILE__
            << ", line:" << __LINE__ << std::endl;
...
static void ggml_backend_sycl_buffer_set_tensor(ggml_backend_buffer_t buffer,
  ...
  SYCL_CHECK(CHECK_TRY_ERROR(
      (*stream).memcpy((char *)tensor->data + offset, data, size).wait()));
} catch (sycl::exception const &exc) {
  std::cerr << exc.what() << "Exception caught at file:" << __FILE__
            << ", line:" << __LINE__ << std::endl;
  std::exit(1);
...
GGML_CALL static const char *ggml_backend_sycl_buffer_type_name(
  ...
  ggml_backend_sycl_buffer_type_context *ctx =
      (ggml_backend_sycl_buffer_type_context *)buft->context;

  return ctx->name.c_str();
}

static ggml_backend_buffer_t ggml_backend_sycl_buffer_type_alloc_buffer(
    ggml_backend_buffer_type_t buft, size_t size) try {
  ...
  void *dev_ptr;
  SYCL_CHECK(
      CHECK_TRY_ERROR(dev_ptr = (void *)sycl::malloc_device(size, *stream)));

  ggml_backend_sycl_buffer_context *ctx =
      new ggml_backend_sycl_buffer_context(device, dev_ptr);

  return ggml_backend_buffer_init(buft, ggml_backend_sycl_buffer_interface, ctx,
                                  size);
} catch (sycl::exception const &exc) {
  std::cerr << exc.what() << "Exception caught at file:" << __FILE__
            << ", line:" << __LINE__ << std::endl;
...
static size_t ggml_backend_sycl_buffer_type_get_alignment(
    ggml_backend_buffer_type_t buft) {
  return 128;
...
static size_t ggml_backend_sycl_buffer_type_get_alloc_size(
...
static bool ggml_backend_sycl_buffer_type_supports_backend(
...
ggml_backend_buffer_type_t ggml_backend_sycl_buffer_type(int device) {
  static struct ggml_backend_buffer_type
      ggml_backend_sycl_buffer_types[GGML_SYCL_MAX_DEVICES];
...
GGML_CALL static const char *ggml_backend_sycl_host_buffer_type_name(
    ggml_backend_buffer_type_t buft) {
...
GGML_CALL static const char *ggml_backend_sycl_host_buffer_name(
    ggml_backend_buffer_t buffer) {
  return GGML_SYCL_NAME "_Host";
...
static void ggml_backend_sycl_host_buffer_free_buffer(
    ggml_backend_buffer_t buffer) {
  ggml_sycl_host_free(buffer->context);
}

static ggml_backend_buffer_t ggml_backend_sycl_host_buffer_type_alloc_buffer(
...
ggml_backend_buffer_type_t ggml_backend_sycl_host_buffer_type() {
  static struct ggml_backend_buffer_type ggml_backend_sycl_buffer_type_host = {
      /* .iface    = */ {
          ...
          /* .alloc_buffer     = */
          ggml_backend_sycl_host_buffer_type_alloc_buffer,
          /* .get_alignment    = */
          ggml_backend_cpu_buffer_type()
              ->iface.get_alignment,
          /* .get_max_size     = */ NULL,  // TODO: return
                                           // device.maxBufferLength
                                           /* .get_alloc_size   = */
          ggml_backend_cpu_buffer_type()
              ->iface.get_alloc_size,
          /* .supports_backend = */
          ggml_backend_cpu_buffer_type()
              ->iface.supports_backend,
          /* .is_host          = */
          ggml_backend_cpu_buffer_type()
              ->iface.is_host,
      },
...
static void ggml_backend_sycl_free(ggml_backend_t backend) {
  ggml_backend_context_sycl *sycl_ctx =
      (ggml_backend_context_sycl *)backend->context;

  delete sycl_ctx;
  delete backend;
}

static ggml_backend_buffer_type_t ggml_backend_sycl_get_default_buffer_type(
...
static ggml_backend_graph_plan_t ggml_backend_sycl_graph_plan_create(
    ggml_backend_t backend, const ggml_cgraph *cgraph) {
...
static void ggml_backend_sycl_graph_plan_free(ggml_backend_t backend,
...
static void ggml_backend_sycl_graph_plan_compute(
...
static bool ggml_backend_sycl_graph_compute(ggml_backend_t backend,
                                            ggml_cgraph *cgraph) {
  ggml_backend_context_sycl *sycl_ctx =
      (ggml_backend_context_sycl *)backend->context;
...
static bool ggml_backend_sycl_supports_op(ggml_backend_t backend,
...
ggml_backend_t ggml_backend_sycl_init(int device) {
  ...
  // not strictly necessary, but it may reduce the overhead of the first
  // graph_compute
  ggml_sycl_set_main_device(device);

  ggml_backend_context_sycl *ctx =
      new ggml_backend_context_sycl{/* .device = */ device};

  ggml_backend_t sycl_backend =
      new ggml_backend{/* .interface = */ ggml_backend_sycl_interface,
                       /* .context   = */ ctx};
...
static ggml_backend_t ggml_backend_reg_sycl_init(const char *params,
                                                 void *user_data) {
  ggml_backend_t sycl_backend =
      ggml_backend_sycl_init((int)(intptr_t)user_data);
  return sycl_backend;
...
int ggml_backend_sycl_reg_devices() {
  ...
  for (int i = 0; i < device_count; i++) {
    ...
    ggml_backend_register(name, ggml_backend_reg_sycl_init,
                          ggml_backend_sycl_buffer_type(i),
                          (void *)(intptr_t)i);
  }
  return device_count;
}
